...
def compound_growth(
    starting_budget: float, profit_per_flip_pct: float, flips_per_day: float, days: int
) -> dict:
    """
...
def print_growth_table():
    ...
    print("\n   Assumptions:")
    print("   - Starting Budget: $50")
    print(f"   - Net Profit per Flip: {NET_PROFIT_PCT}% (after 0.2% fees)")
    print("   - Reinvesting all profits")

    print("\n   " + "-" * 76)
    print(
        f"   {'Flips/Day':<12} {'30 Days':<15} {'90 Days':<15} {'180 Days':<15} {'365 Days':<15}"
    )
    print("   " + "-" * 76)
    ...
    for flips_per_day, label in scenarios:
        ...
        print(
            f"   {flips_per_day:<12} ${d30['ending_budget']:<14.2f} ${d90['ending_budget']:<14.2f} ${d180['ending_budget']:<14.2f} ${d365['ending_budget']:<14.2f}"
        )
    ...
    print(
        f"   {'Month':<10} {'Budget':<12} {'Monthly Gain':<15} {'Cumulative ROI':<15}"
    )
    print("   " + "-" * 50)
