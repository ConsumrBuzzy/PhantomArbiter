...
class ProfitabilityReport:
    ...
    def __str__(self) -> str:
        ...
        basis = f"+{self.basis_pct:.3f}%" if self.basis_pct >= 0 else f"{self.basis_pct:.3f}%"
...
class ProfitabilityMonitor:
    ...
    async def generate_report(self) -> str:
        ...
        if best_opportunity:
            ...
            lines.append(f"     Breakeven: {best_opportunity.hours_to_breakeven:.0f} hours")
            lines.append(f"     Action: Long Spot + Short {best_opportunity.symbol}-PERP")
        else:
            lines.append("  ⚠️ No profitable opportunities at current rates")
            lines.append(f"     (Need funding > 0.01%/8h to overcome {self.TOTAL_ROUND_TRIP*100:.1f}% fees)")
...
async def main():
    parser = argparse.ArgumentParser(description="Phantom Arbiter Profitability Monitor")
    parser.add_argument("--interval", type=int, default=600, help="Update interval in seconds (default: 600)")
    parser.add_argument("--budget", type=float, default=500.0, help="Total budget in USD (default: 500)")
