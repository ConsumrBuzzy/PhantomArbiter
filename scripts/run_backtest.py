...
import sys
import os
import yfinance as yf # For demo data if local not found
import pandas as pd
from backtesting import Backtest
from src.backtesting.adapters import PhantomScalper, PhantomLongtail, PhantomKeltner, PhantomVWAP
...
def run_scalper(data):
    ...
    bt = Backtest(
        ...
        PhantomScalper,
        cash=1000,
        commission=.001, # 0.1% fee (approx Solana swap)
...
def run_strategy(name, strategy_cls, data, optimize=False):
    ...
    print(f"\n🧪 TESTING: {name}")
    bt = Backtest(data, strategy_cls, cash=1000, commission=.001, exclusive_orders=True)
    
    stats = bt.run()
    sharpe = stats['Sharpe Ratio']
    ret = stats['Return [%]']
    print(f"   📊 Res: Sharpe={sharpe:.2f} | Ret={ret:.2f}% | DD={stats['Max. Drawdown [%]']:.2f}%")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mint', type=str, required=True, help='Token Mint Address (Required)')
    parser.add_argument('--days', type=int, default=14, help='Days for OOS split')
    args = parser.parse_args()

    SPLIT_DATE = (datetime.datetime.now() - datetime.timedelta(days=args.days)).strftime("%Y-%m-%d")
    ...
    try:
        ...
        final_stats = Backtest(test_data, winner['strategy'], cash=1000, commission=.001, exclusive_orders=True).run()
        print(final_stats)
        ...
        if final_sharpe > 0:
            import json
            map_file = os.path.join(os.path.dirname(__file__), "config", "strategy_map.json")
            ...
            # Update
            strategy_map[args.mint] = winner['name'].upper() # SCALPER, KELTNER, LONGTAIL
