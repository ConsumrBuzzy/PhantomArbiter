...
def run_stress_test():
    ...
    calibrator = SlippageCalibrator(
        scorer_config=scorer_config, shadow_manager=shadow, min_bps=100, max_bps=800
    )
    ...
    def print_status():
        ...
        gauge_color = {"GREEN": "🟢", "YELLOW": "🟡", "RED": "🔴"}.get(
            calib_status["gauge"], "⚪"
        )

        tip_color = (
            "🟢"
            if cong_status["multiplier"] == 1.0
            else ("🔴" if cong_status["multiplier"] >= 5.0 else "🟡")
        )
