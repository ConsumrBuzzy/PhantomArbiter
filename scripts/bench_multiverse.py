...
def benchmark_graph_operations():
    ...
    try:
        ...
        print(
            "⚠️  Rust extension not found. Run: cd src_rust && maturin develop --release"
        )
        return
    ...
    print(
        f"   Target: <1ms per edge {'✅' if update_time / len(update_edges) < 1 else '❌'}"
    )
...
def benchmark_jupiter_latency():
    ...
    async def run_quote_test():
        ...
        for i in range(5):
            ...
            if quote:
                print(
                    f"   Sample {i + 1}: {latency:.0f}ms | Output: {quote.output_amount:,}"
                )
            else:
...
def benchmark_end_to_end():
    ...
    print("   Target: <250ms for competitive execution")
