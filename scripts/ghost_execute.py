...
def simulate_jupiter_quotes(cycle: Dict) -> List[Dict]:
    ...
    for i in range(len(tokens)):
        ...
        quote = {
            ...
            "output_mint": f"{tokens[next_idx]}_MINT_ADDRESS" if next_idx != 0 else f"{tokens[0]}_MINT_ADDRESS",
            "in_amount": 1_000_000_000 if i == 0 else quotes[-1]["out_amount"],  # 1 SOL or previous output
            "out_amount": int(1_000_000_000 * (1 + 0.001 * (i + 1))),  # Slight gain each leg
...
def main():
    parser = argparse.ArgumentParser(description="Ghost Execution - Dry Run Bundle Testing")
    parser.add_argument("--hops", type=int, default=4, choices=[2, 3, 4, 5],
                        help="Number of hops in the cycle (default: 4)")
    parser.add_argument("--dry-run", action="store_true", default=True,
                        help="Dry run mode (default: True)")
    parser.add_argument("--simulate-failure", action="store_true",
                        help="Simulate a transaction failure")
