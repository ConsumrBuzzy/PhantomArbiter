...
def create_rpc_panel(monitor: DriftMonitor) -> Panel:
    ...
    for name, data in stats.items():
        ...
        latency = f"{data['avg_latency_ms']:.0f}ms" if data['avg_latency_ms'] > 0 else "-"
...
def create_health_panel(monitor: DriftMonitor) -> Panel:
    ...
    return Panel(
        "\n".join(lines),
        title="[bold green]System Health",
        border_style="green"
    )
...
def main():
    ...
    try:
        with Live(create_layout(monitor), console=console, refresh_per_second=1) as live:
            while True:
