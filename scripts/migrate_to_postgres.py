...
def get_sqlite_stats(sqlite_path: str) -> dict:
    ...
    return {
        ...
        "date_range": f"{datetime.fromtimestamp(min_ts)} to {datetime.fromtimestamp(max_ts)}" if min_ts else "N/A"
    }
...
def migrate_data(
    ...
    while True:
        ...
        for row in rows:
            ...
            pg_rows.append((ts, row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8]))
        ...
        try:
            ...
            print(f"\r   ⏳ Progress: {migrated:,}/{total_rows:,} ({pct:.1f}%) | {rate:.0f} rows/s | ETA: {eta:.0f}s  ", end="", flush=True)
...
def main():
    ...
    parser.add_argument('--stats-only', action='store_true',
                        help='Only show statistics, do not migrate')
