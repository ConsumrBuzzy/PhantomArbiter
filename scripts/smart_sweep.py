...
async def main():
    ...
    for mint, amount in all_tokens.items():
        ...
        if usd_val > 0.01:  # Filter absolute dust
            strays.append({"mint": mint, "amount": amount, "usd_val": usd_val})
    ...
    for stray in strays:
        if stray["usd_val"] > 0.20:  # Worth the gas?
            print(
                f"   🔄 Swapping {stray['mint'][:6]}... -> USDC (${stray['usd_val']:.2f})"
            )
            ...
            if result and result.get("success"):
                ...
                print(
                    f"      ❌ Failed: {result.get('error', 'Unknown') if result else 'No result'}"
                )
    ...
    if sol_bal > (target_sol + 0.05):
        ...
        if surplus_usd > 5.0:
            print(
                f"   🔄 High SOL detected. Swapping {surplus_sol:.4f} SOL surplus to USDC..."
            )
