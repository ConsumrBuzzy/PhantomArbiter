...
async def check_infrastructure() -> dict:
    ...
    try:
        ...
        latencies = [
            s["avg_latency"] for s in rpc.stats.values() if s["avg_latency"] > 0
        ]
...
async def check_components() -> dict:
    ...
    for name, module_path, class_name in components:
        try:
            ...
            results[name] = "❌ BROKEN (Class not found)"
        except Exception as e:
...
async def run_diagnostics():
    console.print(
        Panel("[bold cyan]🚀 PHANTOM ARBITER: PRE-FLIGHT DIAGNOSTICS[/bold cyan]")
    )
