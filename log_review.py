...
def print_log_header(log_path: Path):
    ...
    print(f"{Colors.DIM}   Modified: {mtime.strftime('%Y-%m-%d %H:%M:%S')} | Size: {size_kb:.1f} KB{Colors.RESET}")
...
def main():
    ...
    parser.add_argument("--tail", "-t", type=int, metavar="N",
                        help="Show only last N lines of each log")
    parser.add_argument("--grep", "-g", type=str, metavar="PATTERN",
    ...
    print(f"{Colors.BOLD}Summary:{Colors.RESET} {err_symbol} {total_errors} errors | "
