...
def test_quicknode():
    ...
    if not url:
        test_result("QuickNode", False, "No QUICKNODE_RPC_URL in .env (DevNet endpoints won't work for mainnet)")
        return False
...
def test_ankr():
    ...
    if not url:
        ...
        if api_key:
            ...
            print(f"         (Auto-constructed URL from ANKR_API_KEY)")
        else:
    ...
    try:
        ...
        test_result("Ankr", False, f"Unexpected response")
        return False
    except Exception as e:
...
def test_moralis():
    ...
    try:
        ...
        if resp.status_code == 200:
            data = resp.json()
            test_result("Moralis", True, f"Balance endpoint working")
            return True
...
def test_bigquery():
    ...
    try:
        ...
        for row in result:
            if row.test == 1:
                test_result("BigQuery", True, f"Query executed successfully")
                return True
