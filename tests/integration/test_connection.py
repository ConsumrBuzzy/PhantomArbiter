...
PRIVATE_KEY_STRING = os.getenv("SOLANA_PRIVATE_KEY", "YOUR_PRIVATE_KEY_HERE").strip("'\"")  # Strip quotes if present
...
def run_diagnostics():
        ...
        if data.get('pairs') and len(data['pairs']) > 0:
            ...
            if search_data.get('pairs') and len(search_data['pairs']) > 0:
                ...
                for pair in search_data['pairs']:
                    if pair.get('chainId') == 'solana' and 'WIF' in pair.get('baseToken', {}).get('symbol', '').upper():
                        price = pair['priceUsd']
    ...
    try:
        ...
        for jup_url in jupiter_endpoints:
            try:
                ...
                if response.status_code == 200:
                    print(f"   ✅ Success. Jupiter API is reachable.")
                    connected = True
                    break
                elif response.status_code == 400:
                    # 400 can mean the API works but params are wrong - still connected
                    print(f"   ✅ Jupiter API reachable (returned validation error, but connected).")
                    connected = True
                    break
        ...
        if not connected:
            print(f"   ⚠️  Could not connect to Jupiter API. Trading may not work.")
            print(f"   📝 Note: You may need to use a different RPC or VPN.")
    except Exception as e:
    ...
    try:
        ...
        if "YOUR_PRIVATE_KEY" in PRIVATE_KEY_STRING:
            ...
            if sol_balance < 0.05:
                print("   ⚠️  WARNING: Low Balance. You need at least 0.05 SOL for gas fees.")
            else:
