...
async def test_bitquery_depth():
    ...
    async with websockets.connect(
        WS_URL, subprotocols=["graphql-transport-ws"], extra_headers=headers
    ) as ws:
        # Handshake
        await ws.send(
            json.dumps({"type": "connection_init", "payload": {"token": API_KEY}})
        )
        ack = await ws.recv()
        ...
        while asyncio.get_event_loop().time() - start < 10:
            try:
                ...
                if data.get("type") == "next":
                    ...
                    print(
                        f"   Top Bid: {payload['Bids'][0] if payload.get('Bids') else 'None'}"
                    )
                    return
