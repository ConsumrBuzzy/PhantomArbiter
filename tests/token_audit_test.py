...
def run_audit():
    ...
    for mint, info in TEST_TOKENS.items():
        ...
        print(f"{status} {name:<15} | {mint[:8]}... | Expected: {expected.value:<10} | Got: {actual.value}")
...
def test_program_ids():
    ...
    assert SPL_TOKEN_PROGRAM_ID == "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA", "SPL Token ID mismatch!"
    assert TOKEN_2022_PROGRAM_ID == "TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb", "Token-2022 ID mismatch!"
