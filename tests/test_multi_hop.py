...
class TestPodManager:
    ...
    def test_spawn_cycle_pod(self, mock_settings):
        ...
        from src.engine.pod_manager import PodManager, CyclePod
...
class TestExecutionPod:
    ...
    def test_paper_execution(self, mock_settings, sample_cycle_data, market_context):
        ...
        from src.engine.execution_pod import ExecutionPod, ExecutionMode, ExecutionResult
    ...
    def test_profit_threshold_rejection(self, mock_settings, sample_cycle_data, market_context):
        ...
        loop = asyncio.new_event_loop()
        result = loop.run_until_complete(
            pod._execute_opportunity(low_profit_data)
        )
        loop.close()
    ...
    def test_congestion_pause(self, mock_settings, sample_cycle_data, market_context):
        ...
        loop = asyncio.new_event_loop()
        result = loop.run_until_complete(
            pod._execute_opportunity(sample_cycle_data)
        )
        loop.close()
...
class TestJupiterClient:
    ...
    async def test_quote_profit_calculation(self):
        ...
        from src.engine.dex_builders import MultiHopQuoteBuilder, JupiterClient, SwapQuote
