...
def test_og_b():
    ...
    try:
        ...
        feeds = [JupiterFeed(), RaydiumFeed(), OrcaFeed(use_on_chain=False)]
        detector = SpreadDetector(feeds=feeds)
        print(
            "    Detector initialized with Jupiter, Raydium & Orca(use_on_chain=False)."
        )
    ...
    for opp in opps:
        print(
            f"    ✅ {opp.pair:<12}: Spread {opp.spread_pct:.2f}% | Net ${opp.net_profit_usd:.4f}"
        )

    if not opps:
        print("\n❌ NO OPPORTUNITIES FOUND.")
        print(
            "    If scan completed quickly, price fetch might have failed completely."
        )
