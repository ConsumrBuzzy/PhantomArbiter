...
def load_and_label_data(db_path: str = DB_PATH, lookback_days: int = 30) -> Optional[pd.DataFrame]:
    """
    ...
    try:
        ...
        log(f"Loaded {len(trades_df)} trades from journal (ALL tokens kept for cross-learning).")
        ...
        try:
            ...
            def get_dex_trust(symbol):
                mkt_data = SharedPriceCache.get_market_data(symbol, max_age=86400)  # 24h cache ok for training
            ...
            log(f"Added dex_trust_score feature (from cached market data).")
        ...
        if os.path.exists(db_path):
            try:
                ...
                if len(market_df) >= 100:
                    ...
                    agg_market = market_df.groupby('ts_bucket').agg({
                        'close': ['mean', 'std'],
                        'mkt_liquidity': 'mean'
                    }).reset_index()
                    agg_market.columns = ['ts_bucket', 'global_price_mean', 'global_price_std', 'global_liquidity']
                    
                    # Calculate global volatility
                    agg_market['global_volatility'] = agg_market['global_price_std'] / (agg_market['global_price_mean'] + 1e-10)
                    ...
                    trades_df = pd.merge_asof(
                        trades_df,
                        agg_market[['ts_bucket', 'global_rsi', 'global_volatility', 'global_liquidity']],
                        on='ts_bucket',
                        direction='backward',
                    ...
                    trades_df['global_volatility'] = trades_df['global_volatility'].fillna(0.02)
                    trades_df['global_liquidity'] = trades_df['global_liquidity'].fillna(50000)
                    trades_df['log_global_liquidity'] = np.log1p(trades_df['global_liquidity'])
                    
                    market_features_added = True
                    log(f"Added global market features for {len(trades_df)} trades.")
            except Exception as e:
                log(f"Could not add market features (using execution data only): {e}", "WARNING")
        ...
        log(f"   Features: slippage_pct, log_liquidity, is_volatile, global_rsi, global_volatility")
...
def run_retraining_pipeline(force: bool = False, dry_run: bool = False, lookback_days: int = 30) -> bool:
    """
    ...
    if df is None or len(df) < MIN_SAMPLES:
        log(f"Skipping retraining: Insufficient data samples ({len(df) if df is not None else 0} / {MIN_SAMPLES})")
        return False
    ...
    for train_index, test_index in skf.split(X, y):
        ...
        if has_champion:
            try:
                ...
                log(f"   Fold {fold_idx}: Challenger {fold_acc:.1%} vs Champion {champ_acc:.1%}")
            except:
                log(f"   Fold {fold_idx}: Challenger {fold_acc:.1%} (Champion incompatible)")
        else:
    ...
    avg_champion = np.mean(champion_scores) if (champion_scores and has_champion) else 0.0
    
    log(f"[RESULTS] 5-Fold Results: Challenger [{avg_challenger:.2%}] vs Champion [{avg_champion:.2%}]")
    ...
    if not has_champion:
        ...
        log(f"[WINNER] Challenger Wins! (+{avg_challenger - avg_champion:.2%} improvement)")
        should_train_final = True
    elif force:
        log("[WARN] Challenger Validated Lower/Equal, but FORCE enabled. Swapping.")
        should_train_final = True
    else:
        log(f"[FAIL] Challenger failed to beat Champion. Keeping Old Model.")
        return False
    ...
    if should_train_final:
        ...
        metrics = {
             ...
             'cv_folds': cv_folds,
             'train_samples': len(y),
             'feature_importance': dict(zip(valid_cols, final_model.feature_importances_.tolist()))
        }
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="V43.0 Automated ML Retraining Supervisor")
    parser.add_argument('--force', action='store_true', help='Force retraining even if model is recent')
    parser.add_argument('--dry-run', action='store_true', help='Run training but do not save model')
    parser.add_argument('--lookback', type=int, default=30, help='Days of historical data to use (default: 30)')
