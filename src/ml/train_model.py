...
def train_predictor():
    ...
    if len(raw_df) < 200:
        print(
            "   ⚠️ Insufficient data to train (Need > ~200 ticks for candles). Skipping."
        )
        return False
    ...
    print(
        f"   ⚖️ Class balance: {pos_samples} positive / {neg_samples} negative (scale={scale_pos_weight:.2f})"
    )
...
if __name__ == "__main__":
    try:
        ...
        print(
            "   ❌ Missing dependency. Please run: pip install xgboost scikit-learn joblib"
        )
        print(f"   Error: {e}")
