...
class FeatureGenerator:
    ...
    def create_features(self, df, timeframe="1min"):
        ...
        for mint, group in df.groupby("token_mint"):
            ...
            candles["volume"] = (
                group["volume_h1"].resample(timeframe).mean()
            )  # Propagate Vol
            candles["liquidity"] = (
                group["liquidity_usd"].resample(timeframe).min()
            )  # Conservative Liq
            ...
            candles["spread_var"] = (
                (candles["high"] - candles["low"]) / candles["open"]
            ) * 100
            ...
            future_high = (
                candles["high"].rolling(window=LOOKAHEAD).max().shift(-LOOKAHEAD)
            )

            # Label: 1 if future high > current close * (1 + target)
            candles["target"] = (
                future_high > candles["close"] * (1 + TARGET_PROFIT)
            ).astype(int)
...
if __name__ == "__main__":
    ...
    if not feat.empty:
        print(
            feat[
                ["close", "rsi", "rsi_delta", "bar_pressure", "spread_var", "target"]
            ].tail()
        )
        print(
            f"   Positive Samples: {feat['target'].sum()} ({feat['target'].mean() * 100:.1f}%)"
        )
