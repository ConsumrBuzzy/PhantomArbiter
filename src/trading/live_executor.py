...
class SolanaWallet:
    ...
    def __init__(self, private_key_base58: str):
        ...
        try:
            ...
            Logger.info(f"[WALLET] Loaded wallet: {self.public_key[:8]}...{self.public_key[-4:]}")
    ...
    async def get_balance(self, rpc_url: str) -> Dict[str, float]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code == 200:
                ...
                if accounts:
                    usdc_amount = float(accounts[0].get("account", {}).get("data", {}).get("parsed", {}).get("info", {}).get("tokenAmount", {}).get("uiAmount", 0))
...
class JupiterSwapExecutor:
    ...
    async def execute_swap(
        ...
        try:
            ...
            Logger.info(f"[SWAP] Quote: {in_amount} → {out_amount} (impact: {price_impact}%)")
...
class LiveTrader:
    ...
    async def execute_spatial_arb(
        ...
        if amount_usd > self.max_trade_usd:
            return {"success": False, "error": f"Amount ${amount_usd} exceeds max ${self.max_trade_usd}"}
        ...
        if self.require_confirmation and not self._first_trade_confirmed:
            ...
            print(f"   This will execute a REAL transaction!")
...
async def main():
    ...
    parser.add_argument("--check-only", action="store_true", help="Only check wallet balance")
