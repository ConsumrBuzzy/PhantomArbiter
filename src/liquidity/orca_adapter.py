...
class OrcaAdapter:
    ...
    def derive_whirlpool_address(
        self, token_a: str, token_b: str, tick_spacing: int
    ) -> str:
        """
    ...
    def find_whirlpool(self, token_a: str, token_b: str) -> Optional[WhirlpoolState]:
        ...
        for tick_spacing in [64, 8, 128, 1]:
            ...
            if state:
                Logger.success(
                    f"   🐋 [ORCA] Found pool: ts={tick_spacing} ({TICK_SPACINGS.get(tick_spacing, '?')} fee)"
                )
                return state

        Logger.warning(
            f"   🐋 [ORCA] No active pool found for {token_a[:8]}.../{token_b[:8]}..."
        )
        return None
    ...
    def calculate_range_ticks(
        self, current_price: float, range_pct: float, tick_spacing: int = 64
    ) -> Tuple[int, int]:
        """
    ...
    def _sqrt_price_to_price(
        self, sqrt_price: int, decimals_a: int = 9, decimals_b: int = 6
    ) -> float:
        """
    ...
    def get_tick_array_pdas(
        self, pool_address: str, tick_lower: int, tick_upper: int, tick_spacing: int
    ) -> Tuple[str, str, str]:
        """
        ...
        Logger.debug(
            f"   🐋 [ORCA] Tick Arrays: lower={lower_start}, upper={upper_start}"
        )
    ...
    def build_open_position_ix(
        ...
        if not all(
            [tick_array_lower_pda, tick_array_upper_pda, position_mint, position_pda]
        ):
            Logger.error("   🐋 [ORCA] Failed to derive required accounts")
            return None

        Logger.success("   🐋 [ORCA] Accounts ready for open_position")
        ...
        return {
            ...
            "position_mint_secret": position_mint_secret.hex()
            if position_mint_secret
            else "",
    ...
    def build_add_liquidity_ix(
        ...
        """
        Logger.info(
            f"   🐋 [ORCA] Building increase_liquidity for {position_address[:16]}..."
        )
    ...
    def build_update_fees_and_rewards_ix(
        ...
        """
        Logger.info(
            f"   🐋 [ORCA] Building update_fees_and_rewards for {position_address[:16]}..."
        )
    ...
    def build_collect_fees_ix(
        ...
        """
        Logger.info(
            f"   🐋 [ORCA] Building collect_fees for {position_address[:16]}..."
        )
    ...
    def build_close_position_ix(
        self, position_address: str, owner_pubkey: str
    ) -> Optional[Dict[str, Any]]:
        """
        ...
        """
        Logger.info(
            f"   🐋 [ORCA] Building close_position for {position_address[:16]}..."
        )
...
if __name__ == "__main__":
    ...
    if state:
        ...
        lower, upper = adapter.calculate_range_ticks(
            state.price, 1.0, state.tick_spacing
        )
        print(f"   ±1% Range: tick [{lower}, {upper}]")
        print(
            f"   Price Range: [${adapter.tick_to_price(lower):.4f}, ${adapter.tick_to_price(upper):.4f}]"
        )
    else:
