...
from src.liquidity.types import LiquidityParams
...
class LiquidityManager:
    ...
    def __init__(self, simulation_mode: bool = True):
        ...
        signer_mode = (
            ExecutionMode.SIMULATION if simulation_mode else ExecutionMode.PAPER
        )
        ...
        self.harvest_interval = (
            getattr(Settings, "ORCA_HARVEST_INTERVAL_HOURS", 4) * 3600
        )
    ...
    def should_deploy(
        self, regime: MarketRegime, available_capital: float
    ) -> Tuple[bool, str]:
        """
        ...
        if available_capital < self.min_liquidity:
            return (
                False,
                f"Insufficient capital (${available_capital:.2f} < ${self.min_liquidity})",
            )
        ...
        if active_value >= max_allowed:
            return (
                False,
                f"Max liquidity deployed (${active_value:.2f}/${max_allowed:.2f})",
            )
    ...
    def calculate_position_params(
        self, pool_address: str, capital_usd: float, range_pct: float
        ...
        tick_lower, tick_upper = self.adapter.calculate_range_ticks(
            state.price, range_pct, state.tick_spacing
        )
        ...
        Logger.info(
            f"   💧 [LIQUIDITY] Calculated: ±{range_pct}% range, ${capital_usd:.2f}"
        )
    ...
    def harvest_all_fees(self) -> Dict[str, Tuple[float, float]]:
        ...
        if (now - self._last_harvest_time) < self.harvest_interval:
            hours_until = (
                self.harvest_interval - (now - self._last_harvest_time)
            ) / 3600
            Logger.debug(
                f"   💧 [LIQUIDITY] Harvest cooldown: {hours_until:.1f}h remaining"
            )
            return {}
    ...
    def display_positions(self) -> None:
        ...
        for pos in active:
            ...
            print(
                f"   Range: ${pos.price_lower:.4f} - ${pos.price_upper:.4f} (±{pos.range_pct}%)"
            )
    ...
    def run_health_check(self, current_regime: MarketRegime) -> Dict[str, Any]:
        ...
        if current_regime in [MarketRegime.TREND_UP, MarketRegime.TREND_DOWN]:
            ...
            Logger.warning(
                f"   💧 [LIQUIDITY] Regime change detected: {current_regime.value}"
            )
    ...
    def execute_harvesting_cycle(self, current_regime: MarketRegime) -> Dict[str, Any]:
        ...
        if report.get("needs_harvest"):
            ...
            results["fees_harvested"] = sum(sum(fees) for fees in harvested.values())
        ...
        Logger.success(
            f"   💧 [LIQUIDITY] Cycle complete: {results['positions_closed']} closed, ${results['fees_harvested']:.2f} harvested"
        )

        return results

    def is_price_near_boundary(
        self, position_id: str, threshold_pct: float = 0.5
    ) -> Tuple[bool, str]:
        """
