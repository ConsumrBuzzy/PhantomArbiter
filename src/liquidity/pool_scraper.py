...
class OrcaPoolScraper:
    ...
    def discover_top_pools(
        ...
        Logger.info(f"   🔍 [ORCA] Found {len(ranked)} pools (TVL > ${min_tvl_usd/1000:.0f}K)")
    ...
    def display_top_pools(self, limit: int = 5) -> None:
        ...
        for pool in pools:
            tvl = f"${pool.tvl_usd/1e6:.2f}M" if pool.tvl_usd >= 1e6 else f"${pool.tvl_usd/1e3:.0f}K"
            vol = f"${pool.volume_24h_usd/1e6:.2f}M" if pool.volume_24h_usd >= 1e6 else f"${pool.volume_24h_usd/1e3:.0f}K"
            
            print(f"{pool.pair_name:<15} {tvl:>12} {vol:>12} {pool.fee_rate_pct:>7.2f}% {pool.fee_apr_pct:>7.1f}%")
