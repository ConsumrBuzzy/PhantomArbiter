...
class GlobalRiskGovernor:
    ...
    def __init__(self, initial_capital_usd: float = 1000.0):
        # Configuration
        self.max_daily_drawdown_pct = getattr(Settings, 'MAX_DAILY_DRAWDOWN_PCT', 0.10) # 10%
        self.allocations = {
        ...
        Logger.info(f"🏛️ [GOVERNOR] Initialized. Cap: ${initial_capital_usd:.2f}, MaxDD: {self.max_daily_drawdown_pct*100:.1f}%")
    ...
    def can_execute(self, strategy: str, amount_usd: float) -> bool:
        ...
        with self.lock:
            if self.is_halted:
                Logger.warning(f"🛑 [GOVERNOR] Blocked {strategy}: System Halted ({self.halt_reason})")
                return False
                
            if strategy not in self.allocations:
                Logger.warning(f"⚠️ [GOVERNOR] Unknown strategy '{strategy}', allowing (default risk)")
                return True
    ...
    def record_trade(self, strategy: str, pnl_usd: float):
        ...
        with self.lock:
            ...
            if pnl_usd < 0:
                Logger.info(f"📉 [GOVERNOR] {strategy} Loss: ${pnl_usd:.2f} (Daily PnL: ${self.daily_pnl_usd:.2f})")
    ...
    def _check_drawdown(self):
        ...
        if self.daily_pnl_usd < 0:
            ...
            if current_dd_pct >= self.max_daily_drawdown_pct:
                self.is_halted = True
                self.halt_reason = f"Max Daily Drawdown Hit ({current_dd_pct*100:.2f}%)"
