    ...
    async def get_quote(
        ...
        try:
            ...
            async with session.get(f"{self.BASE_URL}/quote", params=params) as resp:
                ...
                dexes = [step.get("swapInfo", {}).get("ammKey", "unknown") for step in route_plan]
        ...
        try:
            ...
            async with session.post(
                ...
                instruction = SwapInstruction(
                    ...
                    min_output_amount=int(quote.output_amount * (1 - quote.slippage_bps / 10000)),
                )
...
class MultiHopQuoteBuilder:
    ...
    async def build_cycle_quotes(
        ...
        for i in range(len(path) - 1):
            ...
            if not quote:
                Logger.warning(f"[MultiHopQuote] Failed to get quote for leg {i}: {input_mint[:8]}→{output_mint[:8]}")
                return None
