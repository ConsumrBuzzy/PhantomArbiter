...
class ShadowAudit:
    ...
    def csv_headers() -> List[str]:
        """CSV column headers."""
        return [
            "timestamp", "mint", "action", "signal_price",
            "paper_fill", "live_fill", "delta_pct", 
            "execution_lag_ms", "paper_slippage", "live_slippage"
        ]
...
class ShadowManager:
    ...
    async def audit_trade(
        ...
        if not paper_result.success or not live_result.success:
            Logger.debug(f"[SHADOW] Skipping audit - Paper: {paper_result.success}, Live: {live_result.success}")
            return None
        ...
        delta_pct = ((live_result.fill_price - paper_result.fill_price) 
                     / paper_result.fill_price) * 100
    ...
    async def audit_trade_from_tasks(
        ...
        try:
            ...
            if isinstance(paper_result, Exception):
                ...
                paper_result = TradeResult.failed(signal_token, signal_action, str(paper_result), "PAPER")
            
            if isinstance(live_result, Exception):
                Logger.error(f"[SHADOW] Live execution failed: {live_result}")
                live_result = TradeResult.failed(signal_token, signal_action, str(live_result), "LIVE")
            
            return await self.audit_trade(
                signal_token, signal_action, signal_price,
    ...
    def get_stats(self) -> Dict[str, float]:
        ...
        avg_delta = self._sum_delta / self._total_audits if self._total_audits > 0 else 0.0
