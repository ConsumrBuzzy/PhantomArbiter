...
class SignalScanner:
    ...
    def scan_signals(
                    ...
                    signals.append(
                        {
                            ...
                            "symbol": getattr(sig, "token", "UNK")[
                                :4
                            ],  # Using Mint as Symbol if unknown
        ...
        for symbol, watcher in combined_watchers.items():
            ...
            if action in ["BUY", "SELL"]:
                ...
                confidence, reason = self._apply_whale_bonus(
                    watcher, confidence, reason
                )
    ...
    def _check_paper_exits(
        self, symbol: str, watcher: Watcher, price: float
    ) -> Optional[Dict]:
    ...
    def _calculate_confidence(self, watcher: Watcher, price: float) -> float:
        ...
        try:
            ...
            rsi = watcher.get_rsi()
            atr = (
                watcher.data_feed.get_atr()
                if hasattr(watcher.data_feed, "get_atr")
                else 0.0
            )
    ...
    def _apply_whale_bonus(
        self, watcher: Watcher, confidence: float, reason: str
    ) -> tuple:
    ...
    def _cache_results(self, watchers: Dict, signals: List[Dict]) -> None:
        ...
        if best_sym:
            self._last_scan_results["best_play"] = {
                "symbol": best_sym,
                "conf": best_conf,
            }
