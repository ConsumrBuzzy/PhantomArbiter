...
class GhostValidator:
    ...
    async def validate_later(
        self,
        cycle_id: str,
        path: list,
        original_profit: float,
        delay_seconds: float = 0.450,
        input_amount: int = 1_000_000_000,
        ...
        try:
            ...
            quotes = await self.quote_builder.build_cycle_quotes(
                path=path, input_amount=input_amount, slippage_bps=30
            )
            ...
            if age > 2.0:
                Logger.warning(
                    f"[Ghost] ⚠️ STALE DATA VOID: Quote is {age:.2f}s old. Rejected."
                )
                return ValidationResult(
                    cycle_id=cycle_id,
                    original_profit_pct=original_profit,
            ...
            if random.random() < 0.10:
                Logger.warning(
                    "[Ghost] 🎲 BUNDLE DROPPED: Simulated Jito Auction Failure (10% chance)"
                )
                return ValidationResult(
                    cycle_id=cycle_id,
                    original_profit_pct=original_profit,
