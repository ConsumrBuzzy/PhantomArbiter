...
class BridgePod(BasePod):
    ...
    def __init__(
        ...
        Logger.info(f"[BridgePod] Initialized (whale_threshold=${whale_threshold_usd:,.0f})")
    ...
    def handle_bridge_event(self, data: Dict[str, Any]):
        ...
        if amount_usd >= self.whale_threshold_usd:
            self.whale_count += 1
            Logger.info(f"🐳 [BridgePod] WHALE DETECTED: ${amount_usd:,.0f} via {protocol} ({signature[:8]})")
            
            # Emit LIQUIDITY_INFLOW signal immediately
            self.emit_signal(PodSignal(
                pod_id=self.id,
