...
class WatcherManager:
    ...
    def _start_scout_init_thread(self) -> None:
        ...
        t = threading.Thread(
            target=init_scouts_bg, 
            daemon=True, 
            name=f"ScoutInit-{self.engine_name}"
        )
        t.start()
                ...
                for symbol, pos_data in positions.items():
                    ...
                    if watcher:
                        ...
                        if not watcher.in_position:
                            ...
                            Logger.info(f"   🔄 [{self.engine_name}] Reconciled: {symbol}")
                    else:
                        # No watcher - create orphan watcher
                        mint = pos_data.get("mint", "")
                        if mint:
                            orphan = Watcher(symbol, mint, validator=self.validator, is_critical=False)
                            ...
                            Logger.warning(f"   ⚠️ [{self.engine_name}] Orphan position recovered: {symbol}")
                            
        except Exception as e:
            priority_queue.add(3, 'LOG', {
                'level': 'ERROR', 
                'message': f"[V47.6] Reconciliation Error: {e}"
            })
