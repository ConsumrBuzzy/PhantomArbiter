...
class HeartbeatReporter:
    ...
    def send_heartbeat(
        ...
        if (
            ...
            and now - self.last_sync_active >= self.SYNC_INTERVAL
        ):
    ...
    def _collect_heartbeat_data(
        self, tick_count: int, watchers: Dict, scout_watchers: Dict
    ) -> HeartbeatData:
        ...
        paper_section = self._format_paper_section(
            watchers, scout_watchers, wallet_state
        )
    ...
    def _format_paper_section(
        self, watchers: Dict, scout_watchers: Dict, wallet_state: Dict
    ) -> str:
    ...
    def _format_cex_section(self) -> str:
        ...
        if not self.dydx_adapter or not getattr(
            self.dydx_adapter, "is_connected", False
        ):
            return ""
