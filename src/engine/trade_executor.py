...
class TradeExecutor:
    ...
    def _check_preflight_buy(
        self, watcher: "Watcher", size_usd: float
    ) -> Tuple[bool, str]:
        """
        ...
        if self.validator and hasattr(watcher, "mint"):
            if self.validator.is_token_2022(watcher.mint):
                ...
                if is_paper_aggressive and getattr(
                    Settings, "PAPER_ALLOW_TOKEN_2022", False
                ):
                    pass  # Allow it with warning logged elsewhere
                else:
                    if is_paper_aggressive:
                        print("      ❌ Pre-flight FAILED: Token-2022 (unsupported)")
        ...
        if not self.live_mode:
            ...
            if is_paper_aggressive:
                print(
                    f"      🔍 Cash Check: ${self.paper_wallet.cash_balance:.2f} >= ${min_buy_size}? {self.paper_wallet.cash_balance >= min_buy_size}"
                )
            if self.paper_wallet.cash_balance < min_buy_size:
                if is_paper_aggressive:
                    print(
                        f"      ❌ Pre-flight FAILED: Insufficient cash (${self.paper_wallet.cash_balance:.2f} < ${min_buy_size})"
                    )
        ...
        if not self.live_mode:
            ...
            if is_paper_aggressive:
                print(
                    f"      🔍 Gas Check: {self.paper_wallet.sol_balance:.4f} SOL >= {min_gas}? {self.paper_wallet.sol_balance >= min_gas}"
                )
            if self.paper_wallet.sol_balance < min_gas:
                ...
                if not refilled or self.paper_wallet.sol_balance < min_gas:
                    if is_paper_aggressive:
                        print(
                            f"      ❌ Pre-flight FAILED: Insufficient SOL for gas ({self.paper_wallet.sol_balance:.4f} < {min_gas})"
                        )
        ...
        min_liq = (
            getattr(Settings, "PAPER_MIN_LIQUIDITY", 100000)
            if is_paper_aggressive
            else 100000
        )
        ...
        if is_paper_aggressive:
            print(
                f"      🔍 Liquidity Check: ${liq:,.0f} >= ${min_liq:,.0f}? {liq >= min_liq or liq == 0}"
            )
        if liq > 0 and liq < min_liq:
            if is_paper_aggressive:
                print(
                    f"      ❌ Pre-flight FAILED: Low Liquidity ${liq / 1000:.1f}k < ${min_liq / 1000:.0f}k"
                )
        ...
        open_count = (
            len(self.paper_wallet.assets) if hasattr(self, "paper_wallet") else 0
        )
        if is_paper_aggressive:
            print(
                f"      🔍 Position Limit: {open_count} < {max_positions}? {open_count < max_positions}"
            )
        if open_count >= max_positions:
            if is_paper_aggressive:
                print(
                    f"      ❌ Pre-flight FAILED: MAX_POSITIONS ({open_count}/{max_positions})"
                )
    ...
    def _check_price_divergence(
        self, watcher: "Watcher", signal_price: float
        ...
        try:
            ...
            if divergence > self.PRICE_DIVERGENCE_TOLERANCE:
                ...
                priority_queue.add(
                    2,
                    "LOG",
                    {"level": "WARNING", "message": f"⚠️ [DIVERGENCE] {symbol}: {msg}"},
                )
            ...
            priority_queue.add(
                3,
                "LOG",
                {"level": "WARNING", "message": f"⚠️ [PYTH] {symbol} check failed: {e}"},
            )
            return True, str(e), 0.0  # Fail open

    def _check_stop_loss(
        self, watcher: "Watcher", current_price: float
    ) -> Tuple[bool, str, float]:
        """
        ...
        if (
            hasattr(watcher, "max_price_achieved")
            and watcher.max_price_achieved > entry_price
        ):
            trailing_clawback_pct = (
                0.15  # V53.0: Tight 15% trail from peak for bonding curves
            )
            ...
            if current_price < trail_price and current_price > entry_price:
                ...
                return (
                    True,
                    f"TRAILING STOP (-{trailing_clawback_pct * 100:.0f}% from High)",
                    current_price,
                )
    ...
    def _apply_ml_filter(
        self, watcher: "Watcher", price: float, liq: float
    ) -> Tuple[bool, float]:
        """
        ...
        try:
            ...
            # 1. Base Metrics
            prices = (
                [p[1] for p in watcher.data_feed.raw_prices]
                if hasattr(watcher.data_feed, "raw_prices")
                else []
            )
            timestamps = (
                [p[0] for p in watcher.data_feed.raw_prices]
                if hasattr(watcher.data_feed, "raw_prices")
                else []
            )
            ...
            recent_ticks = [
                p for i, p in enumerate(prices) if timestamps[i] > window_start
            ]
            ...
            features_v62 = np.array(
                [[rsi, rsi_delta, bar_pressure, spread_var, log_liq, latency]]
            )
    ...
    def _get_execution_price(
        self, watcher: "Watcher", signal_price: float, delay_ms: int
    ) -> float:
        """
    ...
    def execute_buy(
        ...
        if price is None or price <= 0:
            ...
            if price is None or price <= 0:
                return TradeResult.failed(
                    watcher.symbol, "BUY", "No valid price for buy"
                )
        ...
        if is_paper_aggressive:
            print(
                f"      🧠 ML Filter: Prob={ml_prob:.1%} vs Threshold={self.ML_THRESHOLD:.0%} → {'PASS' if ml_passed else 'FAIL'}"
            )
        ...
        if not ml_passed and not is_paper_aggressive:
            ...
            if is_paper_aggressive:
                print("      ❌ ML REJECTED (but bypassed in paper mode)")
            ...
            print("      ⚠️ ML would reject, but bypassing in PAPER_AGGRESSIVE_MODE")

        if getattr(Settings, "DRY_RUN", False):
            if is_paper_aggressive:
                print(
                    "      🎭 DRY_RUN mode intercepting - simulated trade only (DB logging)"
                )
            try:
                # V63.1: Log to DB
                self._log_simulated_trade(
                    "BUY", watcher.symbol, size_usd, price, reason, ml_prob
                )
            except Exception:
                import traceback
            ...
            return TradeResult(
                ...
                watcher.symbol,
                price,
                size_usd / price if price else 0,
                0.0,
                tx_id="sim_tx_id",
                reason=reason,
            )
        ...
        # Global Lock
        if is_paper_aggressive:
            print("      🔐 Requesting portfolio lock...")
        ...
        if execution_result.success:
            ...
            priority_queue.add(
                ...
                {
                    "level": "ERROR",
                    "message": f"❌ BUY FAILED: {execution_result.reason}",
                },
            )
        ...
        if tx_id:
            ...
            # Log success
            Logger.success(
                f"[{self.engine_name}] 🚀 BUY COMPLETE: {watcher.symbol} Tx:{tx_id}"
            )
            time.sleep(1)
            ...
            if (
                self.live_mode
                and self.shadow_manager
                and getattr(Settings, "ENABLE_SHADOW_MODE", False)
                and getattr(Settings, "ENABLE_TRADING", False)
            ):
                # Run paper execution in background/parallel
                try:
                    # We need to manually run paper logic here since we just did live
                    paper_result = self._execute_paper_buy(
                        watcher, price, reason, liq, decision_engine, size_usd=size_usd
                    )
                    ...
                    if loop.is_running():
                        loop.create_task(
                            self.shadow_manager.audit_trade(
                                "BUY", watcher.symbol, price, paper_result, live_result
                            )
                        )
                except Exception as e:
            ...
            priority_queue.add(
                2, "LOG", {"level": "ERROR", "message": f"BUY FAILED: {watcher.symbol}"}
            )
    ...
    def _execute_paper_buy(
        ...
        if random.random() < failure_rate:
            ...
            self.paper_wallet.stats["fees_paid_usd"] += (
                Settings.SIMULATION_SWAP_FEE_SOL * 150
            )
            priority_queue.add(
                3,
                "LOG",
        ...
        if self.JITO_ENABLED:
            ...
            priority_queue.add(
                ...
                {
                    "level": "DEBUG",
                    "message": f"💸 [PAPER] Paid Jito Tip: {jito_tip:.4f} SOL",
                },
            )
        ...
        if not success:
            priority_queue.add(
                2, "LOG", {"level": "WARN", "message": f"⚠️ [PAPER] Buy Failed: {msg}"}
            )
            return TradeResult.failed(watcher.symbol, "BUY", msg)
    ...
    def _log_paper_buy(
        self, symbol: str, price: float, reason: str, size_usd: float
    ) -> None:
    ...
    def execute_sell(
        self, watcher: "Watcher", price: float, reason: str
    ) -> TradeResult:
        """
        ...
        if price is None or price <= 0:
            ...
            if price is None or price <= 0:
                return TradeResult.failed(
                    watcher.symbol, "SELL", "No valid price for sell"
                )
        ...
        if getattr(Settings, "DRY_RUN", False):
            ...
            pnl_pct = (
                ((price - entry_price_log) / entry_price_log) * 100
                if entry_price_log > 0
                else 0
            )
            ...
            # Log success
            Logger.success(
                f"[{self.engine_name}] 🧪 SIMULATED SELL: {watcher.symbol} (PnL: ${pnl_usd:.2f})"
            )
        ...
        if tx_id:
            ...
            msg = TradeTemplates.exit(
                ...
                pnl=pnl_usd,
                pnl_pct=pnl_pct,
                hold_time_mins=(time.time() - watcher.entry_time) / 60
                if (hasattr(watcher, "entry_time") and watcher.entry_time is not None)
                else 0,
                exit_reason=reason,
            )
            ...
            Logger.success(
                f"[{self.engine_name}] {icon} SELL COMPLETE: {watcher.symbol} (PnL: ${pnl_usd:.2f})"
            )
            self.portfolio.release_lock()
            ...
            if pnl_usd > 0:
                ...
                Logger.warning(
                    f"[{self.engine_name}] 📉 LOSS: ${abs(pnl_usd):.2f} - Streak = {self.consecutive_losses}"
                )
            ...
            if (
                ...
                try:
                    ...
                    if loop.is_running():
                        loop.create_task(
                            self.shadow_manager.audit_trade(
                                "SELL", watcher.symbol, price, paper_result, live_result
                            )
                        )
                except Exception as e:
                    Logger.warning(f"[SHADOW] Failed to trigger audit: {e}")
            ...
            priority_queue.add(
                2,
                "LOG",
                {"level": "ERROR", "message": f"SELL FAILED: {watcher.symbol}"},
            )
    ...
    def _execute_paper_sell(
        ...
        # Get liquidity
        data_feed_meta = (
            getattr(watcher.data_feed, "meta", {})
            if hasattr(watcher, "data_feed")
            else {}
        )
        liquidity = (
            data_feed_meta.get("liquidity_usd", 1000000) if data_feed_meta else 1000000
        )
        ...
        if not success:
            priority_queue.add(
                2, "LOG", {"level": "WARN", "message": f"⚠️ [PAPER] Sell Failed: {msg}"}
            )
    ...
    def _log_simulated_trade(
        self, side, symbol, size_usd, price, reason, confidence, pnl=0.0
    ):
        ...
        try:
            ...
            Logger.info(
                f"🧪 [SIMULATION] {side} {symbol} @ ${price:.4f} (${size_usd:.0f}) | {reason} (Conf: {confidence:.2f} PnL: ${pnl:.2f})"
            )
