...
from src.engine.pod_manager import BasePod, PodConfig, PodSignal, PodType, PodStatus
...
class ExecutionPod(BasePod):
    ...
    async def _execute_opportunity(self, opp: Dict[str, Any]) -> Optional[ExecutionResult]:
        """
        ...
        if expected_profit_pct < adjusted_threshold:
            Logger.debug(f"[ExecutionPod] Profit {expected_profit_pct:.3f}% < threshold {adjusted_threshold:.3f}%")
            return ExecutionResult(
                cycle_id=cycle_id,
                success=False,
    ...
    async def _execute_paper(
        ...
        Logger.info(f"[ExecutionPod] 📝 PAPER TRADE: {hop_count}-hop | "
