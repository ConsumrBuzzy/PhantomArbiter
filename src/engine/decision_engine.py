...
class DecisionEngine(BaseStrategy):
    ...
    def update_market_mode(self):
        ...
        Logger.info(f"🧠 DSA Mode: {self.market_mode} (Win Rate: {self.win_rate*100:.1f}% | Trades: {total_trades})")
    ...
    def _evaluate_entry(self, watcher: Watcher, price: float, rsi: float) -> tuple[str, str, float]:
        ...
        if not passes:
            ...
            Logger.debug(f"   🚫 {watcher.symbol}: REJECTED (Slippage {slippage:.2f}% > 2%)")
            return 'HOLD', '', 0.0
        ...
        if volatility > HIGH_VOL_THRESHOLD:
            ...
            Logger.debug(f"   📊 {watcher.symbol}: High Vol ({volatility:.2f}%) - Size reduced to 67%")
        
        # V12.1: Soft slippage filter - halve size if >1% (stacks with V12.2)
        if action == 'HALF_SIZE':
            size_usd = size_usd / 2
            Logger.debug(f"   ⚠️ {watcher.symbol}: Size halved (Slippage {slippage:.2f}% > 1%)")
        ...
        if consecutive_losses >= 5:
            ...
            Logger.debug(f"   🔴 DRAWDOWN: 5+ losses - Size reduced to 25%")
        elif consecutive_losses >= 3:
            size_usd = size_usd * 0.50
            Logger.debug(f"   🟠 DRAWDOWN: {consecutive_losses} losses - Size reduced to 50%")
        
        # C. Checks
        if price < Settings.MIN_PRICE_THRESHOLD:
            if is_paper_diagnostic:
                print(f"      ❌ Price too low: ${price:.6f} < ${Settings.MIN_PRICE_THRESHOLD}")
            return 'HOLD', '', 0.0
        
        if watcher.get_price_count() < Settings.MIN_VALID_PRICES:
            if is_paper_diagnostic:
                print(f"      ❌ Not enough price data: {watcher.get_price_count()} < {Settings.MIN_VALID_PRICES}")
            return 'HOLD', '', 0.0
        
        if watcher.hourly_trades >= Settings.MAX_TRADES_PER_HOUR:
            if is_paper_diagnostic:
                print(f"      ❌ Trade limit reached: {watcher.hourly_trades} >= {Settings.MAX_TRADES_PER_HOUR}")
            return 'HOLD', '', 0.0
        ...
        if is_paper_diagnostic:
            print(f"      ✅ Pre-checks passed (Price: ${price:.6f}, RSI: {rsi:.1f}, Count: {watcher.get_price_count()})")
        ...
        if is_paper_diagnostic:
            print(f"      🎲 RSI Check: {rsi:.1f} < {rsi_threshold}? {rsi < rsi_threshold}")
        ...
        if rsi < rsi_threshold:
            ...
            is_uptrend = TechnicalAnalysis.is_uptrend(price, history, sma_period=20)  # V19.2: SMA 20 (was 50)
            ...
            if is_uptrend:
                ...
                if is_paper_diagnostic:
                    print(f"   ✅ BUY SIGNAL: {watcher.symbol} - RSI {rsi:.1f}<{rsi_threshold}, Uptrend confirmed")
                return 'BUY', f"DSA {self.market_mode} (RSI {rsi:.1f}<{rsi_threshold}) [{info}]", size_usd
            else:
                if is_paper_diagnostic:
    ...
    def _evaluate_exit(self, watcher: Watcher, price: float, rsi: float) -> tuple[str, str, float]:
        ...
        if rsi > 70 and pnl_pct > Settings.FAST_SCALP_PCT:
            if watcher.trailing_stop_price == 0:  # Only if TSL not active
                return 'SELL', f"⚡ FAST SCALP (RSI {rsi:.1f}, +{pnl_pct*100:.2f}%)", 0.0
