...
class PostgresAdapter:
    ...
    def __init__(self, connection_url: str = None):
        ...
        print("🌐 [POSTGRES] Initializing connection pool...")

        # Create connection pool
        self._pool = pool.ThreadedConnectionPool(
            minconn=PG_POOL_MIN_CONN, maxconn=PG_POOL_MAX_CONN, dsn=self.connection_url
        )
        ...
        print(
            f"✅ [POSTGRES] Connection pool ready (min={PG_POOL_MIN_CONN}, max={PG_POOL_MAX_CONN})"
        )
            ...
            with conn.cursor() as cur:
                try:
                    ...
                    try:
                        ...
                        print(
                            "   ⚠️ TimescaleDB not available (using standard PostgreSQL)"
                        )
    ...
    def get_history(
        self, mint: str, start_ts: float = 0, end_ts: float = None, limit: int = 10000
    ) -> List[Tuple]:
        """
    ...
    def get_recent_data(
        self, lookback_hours: int = 24, limit: int = 50000
    ) -> List[Dict]:
        """
