...
class WalletManager:
    ...
    async def check_and_replenish_gas(self, swapper):
        ...
        try:
            ...
            if sol_balance < Settings.GAS_CRITICAL_SOL:
                Logger.warning(
                    f"⛽ CRITICAL GAS ({sol_balance:.4f} SOL)! Initiating Survival Mode...",
                    icon="🚨",
                )
                ...
                if best_token:
                    ...
                    await swapper.recover_gas(
                        input_mint=best_token, amount_usd=Settings.GAS_REPLENISH_USD
                    )
    ...
    def get_token_info(self, mint_str):
        ...
        try:
            ...
            if result and "value" in result:
                ...
                if accounts:
                    return accounts[0]["account"]["data"]["parsed"]["info"][
                        "tokenAmount"
                    ]
        except Exception as e:
    ...
    def get_sol_balance(self):
        ...
        try:
            ...
            resp = requests.post(
                Settings.RPC_URL, json=payload, headers=headers, timeout=5
            )
            data = resp.json()
    ...
    def get_all_token_accounts(self):
        ...
        for program_id in programs:
            ...
            for rpc_url in endpoints[:3]:
                try:
                    resp = requests.post(
                        rpc_url, json=payload, headers=headers, timeout=10
                    )
                    data = resp.json()
    ...
    def get_current_live_usd_balance(self):
        ...
        """
        if not self.keypair:
            return {"total_usd": 0.0, "breakdown": {}, "assets": []}
        ...
        if (
            ...
            and isinstance(self.cached_balance, dict)
        ...
        if sol_balance > 0:
            ...
            if not price:
                price = 150.0  # Conservative fallback
        ...
        try:
            ...
            for mint, amount in all_tokens.items():
                if mint == Settings.USDC_MINT or mint in usdc_mints:
                    continue
                if mint in bridged_eth_mints:
                    continue
        ...
        # Construct Result
        result = {"total_usd": total_usd, "breakdown": breakdown, "assets": held_assets}

        Logger.info(
            f"💰 LIVE WALLET: ${total_usd:,.2f} (USDC: ${usdc_balance:,.0f} | Bags: {len(held_assets)})"
        )
