...
from config.thresholds import CASH_FLOOR_USD, MAX_POSITION_SIZE_PCT
...
class PositionSizer:
    ...
    @staticmethod
    def calculate_size(atr: float, win_rate: float = 0.0, symbol: str = None, 
                       held_symbols: list = None) -> float:
        """
    ...
    def update_tsl(current_price: float, state: dict) -> tuple[dict, bool, str]:
        ...
        if new_state.get('trailing_stop_price', 0) == 0:
            ...
            if profit_pct >= Settings.TSL_ACTIVATION_PCT:
                # Activate TSL
                new_state['trailing_stop_price'] = new_state['max_price_achieved'] * (1 - Settings.TSL_TRAIL_PCT)
