...
class PortfolioManager:
    ...
    def __init__(self, wallet_manager, initial_capital=None):
        ...
        if self.virtual_mode:
            ...
            print(f"   💼 Portfolio Initialized with VIRTUAL CAPITAL: ${self.cash_available:.2f}")
        else:
        ...
        self.blocked_assets = self._load_blocked_tokens() # V5.7 Blocked tokens (Persisted)
        
    def _load_blocked_tokens(self):
        """Load permanently blocked tokens from disk."""
        import os
        import json
        blocked_file = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../config/blocked_tokens.json"))
    ...
    def _fetch_token_symbol(self, mint):
        ...
        if mint == Settings.USDC_MINT: return "USDC"
        ...
        if is_broker_alive():
            ...
            if wallet_state and wallet_state.get("held_assets"):
                ...
                for symbol, data in wallet_state["held_assets"].items():
                    ...
                    print(f"      ✅ {symbol} ({category}): {data.get('balance', 0):.4f} tokens")
        ...
        for mint, balance in all_tokens.items():
            if balance > 0.01:
                ...
                if symbol != "UNTRACKED":
                    ...
                    print(f"      ⚠️ UNTRACKED: {fetched_sym} ({mint[:8]}...): {balance:.4f}")
    ...
    def _get_category(self, symbol: str) -> str:
        ...
        if symbol in Settings.ACTIVE_ASSETS: return "🟢 ACTIVE"
        if symbol in Settings.VOLATILE_ASSETS: return "🟡 VOLATILE"
        if symbol in Settings.WATCH_ASSETS: return "⚪ WATCH"
        if symbol in Settings.SCOUT_ASSETS: return "🔭 SCOUT"
        return "❓ UNKNOWN"
        ...
        for symbol, watcher in watchers.items():
            ...
            if balance > 0:
                ...
                if watcher.load_persisted_entry(token_balance=balance):
                    ...
                    allocated_value += (balance * curr_price) if curr_price > 0 else cost_basis
                else:
                    # Estimate
    ...
    def check_trade_budget(self, requested_size: float) -> bool:
        ...
        for symbol, balance in self.held_assets.items():
            if symbol == "USDC" or symbol == "SOL": continue
        ...
        if current_exposure + requested_size > Settings.MAX_TOTAL_EXPOSURE_USD:
            print(f"   ⚠️ EXPOSURE CAP: Current ${current_exposure:.0f} + ${requested_size:.0f} > ${Settings.MAX_TOTAL_EXPOSURE_USD:.0f}")
            return False
