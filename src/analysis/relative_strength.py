...
class RelativeStrengthAnalyzer:
    ...
    def calculate_rs_ratio(self, asset_price: float, benchmark_price: float, 
                           asset_price_24h: float = 0, benchmark_price_24h: float = 0) -> float:
        """
        ...
        if asset_price_24h > 0 and benchmark_price_24h > 0:
            ...
            benchmark_return = ((benchmark_price - benchmark_price_24h) / benchmark_price_24h) * 100
    ...
    def calculate_rs_momentum(self, symbol: str, current_rs: float) -> float:
        ...
        if momentum < -1:  # Falling
            self.consecutive_falling[symbol] = self.consecutive_falling.get(symbol, 0) + 1
        else:
    ...
    def analyze_portfolio(self, held_symbols: List[str]) -> List[RSRanking]:
        ...
        def composite_score(r: RSRanking) -> float:
            ...
            return (r.rs_vs_benchmark * 0.4) + (r.rs_momentum * 0.3 * 10) + (r.win_rate * 0.3)
    ...
    def should_early_exit(self, symbol: str, held_symbols: List[str]) -> Tuple[bool, str]:
        """
    ...
    def get_ranking_for_symbol(self, symbol: str, held_symbols: List[str]) -> Optional[RSRanking]:
    ...
    def display_analysis(self, rankings: List[RSRanking]):
        ...
        # Display rankings
        Logger.info(f"   {'Rank':<5} {'Symbol':<10} {'Price':<12} {'RS Ratio':<10} {'Momentum':<12} {'WR':<8}")
        Logger.info("   " + "-" * 60)
        
        for r in rankings:
            momentum_str = r.momentum_direction
            leader_str = "⭐" if r.is_leader else "  "
            Logger.info(f"   {r.rank:<5} {r.symbol:<10} ${r.price:<11.4f} {r.rs_vs_benchmark:<10.1f} {momentum_str:<12} {r.win_rate:.1f}%{leader_str}")
        ...
        Logger.info("")
        Logger.info(f"   Leaders (>100 RS): {len(leaders)} | Laggards (<100 RS): {len(laggards)}")
        
        # Exit candidates
        exit_candidates = self.get_exit_candidates(rankings)
        if exit_candidates:
            Logger.warning(f"   ⚠️ Exit Candidates: {[r.symbol for r in exit_candidates]}")
