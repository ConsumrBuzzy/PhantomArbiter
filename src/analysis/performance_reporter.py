...
class PerformanceReporter:
    ...
    def get_model_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result["last_retrain"] = datetime.fromtimestamp(mtime).strftime("%Y-%m-%d %H:%M")
            ...
            if hasattr(model, 'feature_importances_'):
                ...
                if feature_names is None:
                    ...
                    feature_names = ['slippage_pct', 'log_liquidity', 'is_volatile', 
                                    'global_rsi', 'global_volatility', 'log_global_liquidity']
                ...
                for engine_name in cm.state.get("engines", {}):
                    ...
                    if engine:
                        ...
                        if peak > 0:
                            ...
                            if result["max_drawdown_pct"] is None or dd > result["max_drawdown_pct"]:
    ...
    def generate_report(self, include_header: bool = True) -> str:
        ...
        if model["model_exists"]:
            if model["champion_accuracy"]:
                lines.append(f"   • Champion Accuracy: {model['champion_accuracy']:.1f}%")
            lines.append(f"   • Last Retrain: {model['model_age_hours']}h ago")
            if model["feature_importance"]:
                top_features = ", ".join([
                    f"{k} ({v}%)" for k, v in list(model["feature_importance"].items())[:3]
                ])
        ...
        if execution["trade_count"] > 0:
            ...
            if execution["avg_hold_time_min"] is not None:
                lines.append(f"   • Avg Hold Time: {execution['avg_hold_time_min']:.1f} min")
            if execution["filter_rate_pct"] is not None:
                lines.append(f"   • Safety Gate Filter: {execution['filter_rate_pct']:.1f}% ({execution['signals_blocked']}/{execution['signals_total']})")
            if execution["avg_liquidity_usd"]:
                lines.append(f"   • Avg Liquidity: ${execution['avg_liquidity_usd']:,.0f}")
        else:
        ...
        if financial["trade_count"] > 0:
            lines.append(f"   • Win Rate: {financial['win_rate_pct']:.1f}% ({financial['wins']}W/{financial['losses']}L)")
            if financial["profit_factor"]:
                pf_str = f"{financial['profit_factor']:.2f}" if financial["profit_factor"] != float('inf') else "∞"
            ...
            if financial["max_drawdown_pct"] is not None:
                lines.append(f"   • Max Drawdown: -{financial['max_drawdown_pct']:.1f}%")
        else:
            lines.append("   • No trades in period")
