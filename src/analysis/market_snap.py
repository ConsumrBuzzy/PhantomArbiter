        ...
        try:
            ...
            for mint, data in market_data.items():
                if isinstance(data, dict):
                    ...
                    price = prices_cache.get(symbol, {}).get("price", 0) if isinstance(prices_cache.get(symbol), dict) else 0
                    if change != 0:
            ...
            g_fmt = "\n".join([f"{conf_icon(conf)} {s:<6} {fmt_price(p):<10} +{c:>5.1f}%" for s, c, conf, p in top_g]) if top_g else "_No Gainers_"
            l_fmt = "\n".join([f"{conf_icon(conf)} {s:<6} {fmt_price(p):<10} {c:>6.1f}%" for s, c, conf, p in top_l]) if top_l else "_No Losers_"
            grey_fmt = " ".join([f"{s}({c:+.1f}%)" for s, c, conf, p in top_grey]) if top_grey else "_None_"
            
            # Dashboard One-Liners (Compact for table - no icons for space)
            g_line = " ".join([f"{s}+{c:.0f}%" for s, c, conf, p in top_g[:3]]) if top_g else "—"
            l_line = " ".join([f"{s}{c:.0f}%" for s, c, conf, p in top_l[:3]]) if top_l else "—"
