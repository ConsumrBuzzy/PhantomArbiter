...
class InventoryWidget(DataTable):
    ...
    def on_mount(self):
        ...
        self.add_columns("Token", "Amount", "Entry ($)", "Mark ($)", "Value ($)", "Unrealized PnL", "Status")
    ...
    def update_inventory(self):
        ...
        for symbol, amt in current_holdings.items():
            if amt <= 0.0001: continue # Dust filter
            ...
            status = "HOLD"
            if pnl_pct > 10: status = "🚀 MOON"
            elif pnl_pct < -5: status = "⚠️ RISK"
