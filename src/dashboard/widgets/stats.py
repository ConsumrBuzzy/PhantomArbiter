...
class StatsPanel(Static):
    ...
    def compose(self) -> ComposeResult:
        # Title
        yield Static(
            "⚡ Phantom Arbiter Core V2.0", id="stats_title", classes="panel_header"
        )
    ...
    def update_stats(self):
        ...
        self.query_one("#stat_cycles").update(
            f"Pathfinder: {state.stats['cycles_per_sec']} cps"
        )
        self.query_one("#stat_latency").update(
            f"WSS Latency: {state.stats['wss_latency_ms']}ms"
        )
        self.query_one("#stat_pnl").update(
            f"Global PnL: {state.stats['total_pnl_sol']:.4f} SOL"
        )
