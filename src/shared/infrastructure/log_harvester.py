...
class FailureTracker:
    ...
    def _maybe_emit_spike_alert(
        self, pool_address: str, failure_count: int, error_type: str
    ):
        ...
        Logger.info(
            f"🔥 [FailureTracker] SPIKE: {pool_address[:8]}... | {failure_count} failures in {self.window_seconds}s"
        )
    ...
    def _emit_recoil_signal(self, pool_address: str):
        ...
        Logger.info(
            f"⚡ [FailureTracker] RECOIL: {pool_address[:8]}... | Pressure released - strike window!"
        )
...
class LogHarvester:
    ...
    def start(self):
        ...
        self._thread = threading.Thread(
            target=self._run_forever, daemon=True, name="LogHarvester"
        )
        self._thread.start()
    ...
    async def _connect_and_listen(self):
        ...
        while self.is_running:
            try:
                ...
                Logger.warning(
                    f"👁️ [HARVESTER] Connection Lost: {e}. Reconnecting in {self.reconnect_delay}s..."
                )
    ...
    async def _process_message(self, data: dict):
        ...
        for log in logs:
            ...
            if (
                is_initialize2
                and "Program 675kPX9MCyJsD5ippTu671dKKkCtSE5v4RBmZJtHNv9v success"
                in log
            ):
        ...
        if is_bridge and amount_usd > 0:
            Logger.info(
                f"👁️ [HARVESTER] 🌉 BRIDGE INFLOW: ${amount_usd:,.0f} via {protocol}"
            )
