...
class BitqueryAdapter:
    ...
    def get_first_100_buyers(self, mint: str) -> List[str]:
        ...
        try:
            ...
            resp = requests.post(
                self.REST_URL, json=payload, headers=headers, timeout=15
            )

            if resp.status_code != 200:
                Logger.debug(
                    f"[BITQUERY] REST Query failed: {resp.status_code} - {resp.text}"
                )
                return []
            ...
            if wallets:
                Logger.info(
                    f"🏹 [BITQUERY] Found {len(wallets)} early buyers for {mint[:8]}"
                )
    ...
    async def start(self):
        ...
        if not self.api_key:
            Logger.warning(
                "⚠️ [BITQUERY] No API Key provided. Bitquery Adapter disabled."
            )
            return
            ...
            try:
                ...
                async with websockets.connect(
                    ...
                    payload = {
                        "id": "1",
                        "type": "subscribe",
                        "payload": {"query": self.subscription_query},
                    }
