...
class RPCBalancer:
    ...
    def get_winner(self) -> Optional[RPCProvider]:
        ...
        return next(
            (p for p in self.providers if p.is_leading), self._select_provider()
        )
    ...
    def get_secondary_leader(self, exclude: str = "public") -> RPCProvider:
        ...
        elite = [
            p for p in available if p.is_elite and exclude.lower() not in p.name.lower()
        ]

        if elite:
            # Pick best elite by latency
            return min(
                elite, key=lambda p: p.last_latency_ms if p.last_latency_ms > 0 else 999
            )
    ...
    def call(
        self, method: str, params: Optional[List] = None, retries: int = None
        ...
        for attempt in range(retries):
            ...
            try:
                response = requests.post(
                    provider.url, json=payload, timeout=self.REQUEST_TIMEOUT
                )
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if slot > best_slot:
                        ...
                        slot == best_slot
                        and leader
                        and latency < leader.last_latency_ms
                    ):
                        leader = p
            except:
                continue
            ...
            "providers": [
                {
                    ...
                    "success_rate": f"{(p.success_count / p.request_count * 100):.1f}%"
...
if __name__ == "__main__":
    ...
    for p in stats["providers"]:
        print(
            f"   {p['name']}: {p['status']} | Requests: {p['requests']} | Success: {p['success_rate']}"
        )
