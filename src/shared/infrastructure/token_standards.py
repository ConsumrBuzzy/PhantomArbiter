...
def detect_token_standard(
    mint: str, rpc_response: Optional[Dict] = None
) -> TokenStandard:
    """
        ...
        if response and "result" in response:
            ...
            for ext in extensions:
                if ext.get("extension") == "transferFeeConfig":
                    ...
                    fee_bps = state.get("newerTransferFee", {}).get(
                        "transferFeeBasisPoints", 0
                    )
                    if fee_bps > 0:
                        _known_tax_tokens[mint] = fee_bps
                        Logger.info(
                            f"⚠️ [TOKEN] {mint[:8]}... has {fee_bps / 100:.1f}% transfer fee!"
                        )
                        return True, fee_bps
