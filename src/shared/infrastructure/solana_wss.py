...
class SolanaWSS:
    ...
    async def subscribe_account(
        self, pubkey: str, callback: Callable, encoding: str = "jsonParsed"
    ) -> Optional[int]:
        """
    ...
    async def subscribe_logs(
        self, mentions: List[str], callback: Callable
    ) -> Optional[int]:
        """
        ...
        if result and "result" in result:
            sub_id = result["result"]
            self._subscriptions[sub_id] = Subscription(
                sub_id=sub_id, method="logsSubscribe", params=params, callback=callback
            )
