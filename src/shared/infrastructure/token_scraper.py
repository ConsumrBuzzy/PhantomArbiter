...
class TokenScraper:
    ...
    def __init__(self):
        ...
        Logger.info(
            f"[SCRAPER] Token Scraper V73.0 Initialized (DB: {len(self.cache)}, Jupiter: {len(self.jupiter_list)})"
        )
    ...
    def _load_from_db(self):
        ...
        try:
            ...
            with db_manager.cursor() as c:
                c.execute(
                    "SELECT mint, symbol, name, price, liquidity, volume_24h, dex, source, first_seen, last_seen FROM tokens"
                )
                rows = c.fetchall()
                for row in rows:
                    self.cache[row[0]] = {
    ...
    def lookup(self, mint: str) -> Dict:
                ...
                if resp.status_code == 200:
                    ...
                    if "result" in data:
                        ...
                        if symbol and symbol != "":
                            ...
                            self._save_to_db(mint, result)
                            Logger.debug(
                                f"[SCRAPER] Helius DAS: Found {symbol} for {mint[:8]}"
                            )
                            return result
        except Exception as e:
                ...
                if resp.status_code == 200:
                    ...
                    if data.get("success") and data.get("data"):
                        ...
                        if symbol and symbol != "":
                            ...
                            self._save_to_db(mint, result)
                            Logger.debug(
                                f"[SCRAPER] Birdeye: Found {symbol} for {mint[:8]}"
                            )
                            return result
        except Exception as e:
        ...
        try:
            ...
            if cmc_key:
                ...
                params = {"address": mint}
                resp = requests.get(
                    self.CMC_URL, headers=headers, params=params, timeout=5
                )
                self.last_request = time.time()
                    ...
                    if data.get("status", {}).get("error_code") == 0:
                        ...
                        if tokens:
                            ...
                            if symbol:
                                ...
                                self._save_to_db(mint, result)
                                Logger.debug(
                                    f"[SCRAPER] CMC: Found {symbol} for {mint[:8]}"
                                )
                                return result
        except Exception as e:
            ...
            if resp.status_code == 200:
                ...
                if title_match:
                    ...
                    if name_match:
                        ...
                        self._save_to_db(mint, result)
                        Logger.debug(
                            f"[SCRAPER] Solscan Scrape: Found {symbol} for {mint[:8]}"
                        )
                        return result
        except Exception as e:
    ...
    def get_all_tokens(self) -> List[Dict]:
        ...
        try:
            ...
            with db_manager.cursor() as c:
                c.execute(
                    "SELECT mint, symbol, name, price, liquidity, volume_24h, first_seen, last_seen FROM tokens ORDER BY last_seen DESC"
                )
                rows = c.fetchall()
                return [
                    {
