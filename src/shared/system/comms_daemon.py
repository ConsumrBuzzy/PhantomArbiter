...
class CommsDaemon:
    ...
    def start(self):
        ...
        self.worker_thread = threading.Thread(
            target=self._daemon_loop, daemon=True, name="CommsDaemon"
        )
        ...
        while self.running:
            try:
                ...
                if self.pending_batch:
                    ...
                    if (
                        now - oldest >= self.BATCH_WINDOW_S
                        and now - self.last_send_time >= self.MIN_INTERVAL_S
                    ):
                        self._send_batch(notifier)
            ...
            for attempt in range(self.MAX_RETRIES):
                try:
                    ...
                    if "400" in str(e) or "429" in str(e):
                        Logger.warning(
                            f"[COMMS] Rate limit/Error, backing off {self.ERROR_BACKOFF_S}s"
                        )
