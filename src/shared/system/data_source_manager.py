...
class DataSourceManager:
        ...
        if not self.use_fallback:
            try:
                ...
                if results:
                    ...
                    Logger.warning(
                        f"[DSM] Tier 1 Failed ({self.consecutive_failures}/{self.failure_threshold})"
                    )
            ...
            if self.consecutive_failures >= self.failure_threshold:
                Logger.warning(
                    f"[DSM] Switching to Tier 2 (DexScreener) for {self.fallback_duration}s"
                )
    ...
    def _record_tier1_failure(self):
        ...
        if len(self.tier1_failures) >= self.FAILURE_THRESHOLD:
            ...
            Logger.warning(
                "   🚫 DSM: Tier 1 BLACKLISTED for 2 hours (3 failures in 15min)"
            )
    ...
    def _fetch_tier1(self, mints: list) -> dict:
        ...
        for chunk in chunks:
            ids = ",".join(chunk)
            data = self.router.get_jupiter_price(
    ...
    def get_metrics(self, mint: str) -> dict:
        ...
        return {"liquidity": liquidity, "slippage": slippage, "slippage_action": action}
