...
class ConfigManager:
    ...
    def _prompt_user_menu() -> SessionContext:
        ...
        mode = questionary.select(
            ...
            choices=[
                ...
                questionary.Choice(
                    "👻 GHOST (Simulated Jito + Verification)", value="GHOST"
                ),
        ...
        risk = questionary.select(
            ...
            choices=[
                questionary.Choice(
                    "🛡️ Conservative (Low Size, High Confidence)", value="CONSERVATIVE"
                ),
                questionary.Choice("⚖️ Balanced (Standard)", value="BALANCED"),
                questionary.Choice(
                    "⚔️ Aggressive (Max Size, Speed)", value="AGGRESSIVE"
                ),
            ],
        ).ask()
    ...
    def _check_latency():
        ...
        try:
            ...
            if latency_ms > 500:
                console.print(
                    f"[bold red]⚠️ WARNING: High Network Latency Detected: {latency_ms:.0f}ms[/bold red]"
                )
                console.print(
                    "[red]   Ghost results may be overly optimistic due to lag.[/red]"
                )
                time.sleep(1)  # Let user see warning
            else:
                console.print(
                    f"[green]✅ Network Latency: {latency_ms:.0f}ms (Nominal)[/green]"
                )

        except Exception:
            console.print(
                "[yellow]⚠️ Could not verify latency. Assuming offline/restricted.[/yellow]"
            )
