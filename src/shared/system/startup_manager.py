...
# Services
from src.services.ml_scheduler import ml_retraining_loop, set_shutdown_event as set_ml_shutdown
from src.services.discovery_service import discovery_monitor_loop, set_shutdown_event as set_discovery_shutdown
from src.services.liquidity_service import liquidity_cycle_loop, set_shutdown_event as set_liquidity_shutdown
from src.services.reporting_service import performance_reporting_loop, set_shutdown_event as set_reporting_shutdown
...
class StartupManager:
    ...
    async def launch_services(self) -> List[asyncio.Task]:
        ...
        # 2. Landlord Monitor
        if self.landlord:
            t = loop.create_task(self.landlord.run_monitoring_loop(), name="Landlord_Monitor")
            self.tasks.append(t)
            
        # 3. ML Scheduler (On-Demand)
        self.tasks.append(loop.create_task(ml_retraining_loop(self.broker), name="ML_Monitor"))
        
        # 4. Performance Reporter (6h) & Heartbeat (5m)
        from src.services.reporting_service import performance_reporting_loop, heartbeat_loop
        self.tasks.append(loop.create_task(performance_reporting_loop(), name="Perf_Report"))
        self.tasks.append(loop.create_task(heartbeat_loop(self.broker), name="Heartbeat_Loop"))
        
        # 5. Liquidity Cycle
        self.tasks.append(loop.create_task(liquidity_cycle_loop(), name="Liquidity_Cycle"))
    ...
    async def wait_for_shutdown(self):
        ...
        if self.landlord: self.landlord._running = False
