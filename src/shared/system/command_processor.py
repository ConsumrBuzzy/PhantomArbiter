...
class CommandProcessor:
        ...
        try:
            if cmd_type == "CMD_STATUS_REPORT":
                ...
                try:
                    ...
                    send_telegram(
                        f"❌ Status Error: {e}", source="STATUS", priority="HIGH"
                    )
                ...
                send_telegram(
                    f"⚙️ MARKET MODE: {status}", source="BROKER", priority="HIGH"
                )
                ...
                try:
                    ...
                    send_telegram(
                        f"📉 Position Size Updated: ${amount:.2f}",
                        source="BROKER",
                        priority="HIGH",
                    )
                except:
                ...
                try:
                    ...
                    send_telegram(
                        f"💰 Risk Budget Updated: ${amount:.2f}",
                        source="BROKER",
                        priority="HIGH",
                    )
                except:
    ...
    def _handle_test_cex(self):
        ...
        if not agg:
            send_telegram(
                "❌ MarketAggregator missing", source="BROKER", priority="HIGH"
            )
            return
        ...
        if not adapter or not adapter.is_connected:
            send_telegram(
                "❌ dYdX Adapter not connected", source="BROKER", priority="HIGH"
            )
            return
        ...
        if result.get("success"):
            ...
            send_telegram(
                f"❌ CEX Test Failed: {result.get('error')}",
                source="BROKER",
                priority="HIGH",
            )
    ...
    def _handle_test_drift(self):
        ...
        try:
            ...
            if result.get("success"):
                ...
                send_telegram(
                    f"❌ Drift Test Failed: {result.get('error')}",
                    source="BROKER",
                    priority="HIGH",
                )
        except Exception as e:
    ...
    def _handle_check_drift(self):
        ...
        try:
            ...
            send_telegram(
                f"{status}\nCollateral: ${result.get('collateral', 0):.2f}",
                source="BROKER",
                priority="HIGH",
            )
        except Exception as e:
            send_telegram(
                f"❌ Drift Check Error: {e}", source="BROKER", priority="HIGH"
            )
    ...
    def _handle_start_landlord(self, cmd_string):
        ...
        try:
            from src.legacy.strategies.components.landlord_core import get_landlord
    ...
    def _handle_set_jlp(self, cmd_string):
        ...
        try:
            ...
            send_telegram(
                f"🏠 JLP Set: {qty} @ ${price}", source="BROKER", priority="HIGH"
            )
        except:
