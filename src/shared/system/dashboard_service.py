...
class DashboardService:
    ...
    def start(self):
        ...
        self.thread = threading.Thread(
            target=self._run_loop, daemon=True, name="DashboardService"
        )
        self.thread.start()
    ...
    def _collect_state(self) -> DashboardState:
        ...
        if self.broker:
            ...
            if hasattr(self.broker, "scout_agent"):
                ...
                state.scout_watchlist = (
                    len(scout.watchlist) if hasattr(scout, "watchlist") else 0
                )
            ...
            if hasattr(self.broker, "sauron"):
                state.sauron_discoveries = getattr(
                    self.broker.sauron, "discovery_count", 0
                )
        ...
        try:
            ...
            if sol_price:
                ...
                router = SmartRouter()
                sol_price = router.get_jupiter_price(
                    "So11111111111111111111111111111111111111112", Settings.USDC_MINT
                )
                ...
                state.sol_price = sol_price
                state.sol_is_cached = (
                    False if sol_price else True
                )  # Live if Jupiter worked
        except:
        ...
        try:
            ...
            for engine_name in cm.state.get("engines", {}):
                ...
                if engine:
                    ...
                    state.gas_value_usd = (
                        state.sol_balance * state.sol_price if state.sol_price else 0
                    )
                    ...
                    state.total_value = (
                        state.cash_balance + state.gas_value_usd + pos_value
                    )
                    ...
                    for symbol, pos in positions.items():
                        ...
                        # Calculate PnL %
                        pnl_pct = (
                            ((current_value - size_usd) / size_usd * 100)
                            if size_usd > 0
                            else 0
                        )
    ...
    def _print_dashboard(self, state: DashboardState):
        ...
        if getattr(Settings, "SILENT_MODE", False) or getattr(
            Logger, "_silent_mode", False
        ):
            return
        ...
        regime_emoji = (
            "📈" if "UP" in state.regime else "📉" if "DOWN" in state.regime else "↔️"
        )
        ...
        total_val = (
            ...
            else (state.cash_balance + state.gas_value_usd)
        )

        # V80.0: Exposure bar
        exp_bar = "█" * int(state.exposure_pct / 10) + "░" * (
            10 - int(state.exposure_pct / 10)
        )
        ...
        if state.positions_list:
            lines.append("├" + "─" * 68 + "┤")
            lines.append(
                f"│ 📊 POSITIONS ({state.open_positions}) │ Exposure: [{exp_bar}] {state.exposure_pct:.0f}%         │"
            )
            lines.append(
                "├"
                + "─" * 10
            ...
            for pos in state.positions_list[:5]:  # Show top 5
                ...
                lines.append(
                    f"│ {pos['symbol']:<8} │ ${pos['entry']:<10.4f} │ ${pos['current']:<10.4f} │ ${pos['size']:<7.2f} │ {pnl_bar} {pnl_str:<16} │"
                )

            if len(state.positions_list) > 5:
                lines.append(
                    f"│ ... and {len(state.positions_list) - 5} more positions                                        │"
                )
