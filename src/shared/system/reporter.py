import time
from src.shared.system.comms_daemon import send_telegram_chunked
...
class MarketReporter:
    ...
    def send_market_snapshot(
        self, batch_prices: dict, wss_stats: dict, priority: str = "LOW"
    ):
        """
        ...
        if self.market_aggregator:
            try:
                ...
                if adapter and adapter.is_connected:
                    ...
                    if eth:
                        cex_lines.append(f"   ETH: ${eth['price']:,.2f}")
                    if btc:
                        cex_lines.append(f"   BTC: ${btc['price']:,.2f}")
                    if sol:
                        cex_lines.append(f"   SOL: ${sol['price']:,.4f}")
    ...
    def _calculate_movers(self, current_prices: dict, exclude_symbols: set) -> list:
        ...
        for symbol, price in current_prices.items():
            ...
            # Formatting
            liq_str = (
                f"${liq / 1_000_000:.1f}M"
                if liq > 1_000_000
                else f"${liq / 1_000:.0f}K"
                if liq > 1000
                else f"${liq:.0f}"
            )
