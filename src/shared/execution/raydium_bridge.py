...
class RaydiumBridge:
    ...
    def _send_command(
        self, cmd_data: dict, timeout: int = 30
    ...
    def get_quote(
        self, pool_address: str, input_mint: str, amount: float
    ) -> RaydiumQuoteResult:
        """
    ...
    def get_price(self, pool_address: str) -> RaydiumPriceResult:
        ...
        """
        result = self._send_command({"cmd": "price", "pool": pool_address})
    ...
    def get_batch_prices(self, pools: list) -> Dict[str, float]:
        ...
        for i in range(0, len(pools), BATCH_LIMIT):
            ...
            resp = self._send_command(
                {"cmd": "batch_prices", "pools": batch}, timeout=60
            )
    ...
    def execute_swap(
        self, pool_address: str, input_mint: str, amount: float, slippage_bps: int = 50
    ) -> RaydiumSwapResult:
        """
    ...
    def discover_pool(self, token_a: str, token_b: str) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            result = subprocess.run(
                cmd, capture_output=True, text=True, cwd=str(self.bridge_path.parent)
            )
            ...
            if data.get("success"):
                Logger.debug(
                    f"[RAYDIUM] Discovered pool {data.get('poolId')} ({data.get('type', 'unknown')})"
                )
            return data
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if data.get("success"):
                    Logger.debug(
                        f"[RAYDIUM] Found Standard/AMMX Route via API for {mint_a[:6]}"
                    )
    ...
    async def fetch_api_quote(
        self, input_mint: str, output_mint: str, amount: float, slippage_bps: int = 50
    ) -> Optional[Dict[str, Any]]:
        """
    ...
    def execute_swap_rust(
        ...
        try:
            ...
            if not payer_keypair:
                return RaydiumSwapResult(
                    False, None, input_mint, "", amount, 0, "No Payer Keypair"
                )
            ...
            if not resp_pool.get("result") or not resp_pool["result"]["value"]:
                return RaydiumSwapResult(
                    False, None, input_mint, "", amount, 0, "Pool account not found"
                )

            account_data_b64 = resp_pool["result"]["value"]["data"][0]

            # 2. Parse State (Rust)
            pool_info = phantom_core.parse_clmm_pool_state(
                pool_address, account_data_b64
            )
            ...
            tick_arrays = phantom_core.derive_tick_arrays(
                pool_address, pool_info.tick_current, pool_info.tick_spacing, is_a_to_b
            )

            # 5. Build Instruction (The Forge)
            decimals = (
                pool_info.mint_decimals_0 if is_a_to_b else pool_info.mint_decimals_1
            )
            ...
            out_decimals = (
                pool_info.mint_decimals_1 if is_a_to_b else pool_info.mint_decimals_0
            )
            ...
            if "error" in resp_send:
                return RaydiumSwapResult(
                    ...
                    amount,
                    0,
                    f"RPC Error: {resp_send['error']}",
                )

            signature = resp_send.get("result")
            return RaydiumSwapResult(
                True, signature, input_mint, output_mint, amount, 0, None
            )
...
def get_pool_for_pair(mint_a: str, mint_b: str) -> str:
    ...
    for pair, pool in RAYDIUM_CLMM_POOLS.items():
        ...
        if len(tokens) == 2:
            ...
            if (tokens[0] == "SOL" and mint_a == SOL_MINT) or (
                tokens[0] == "USDC" and mint_a == USDC_MINT
            ):
                return pool
    return ""
