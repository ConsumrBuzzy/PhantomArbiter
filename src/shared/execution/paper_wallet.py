...
class PaperWallet:
    ...
    def __init__(self, engine_name: str = "PRIMARY"):
        ...
        # V45.5: Auto-register engine if missing (Dynamic Engines support)
        if hasattr(self.cm, "state") and engine_name not in self.cm.state.get(
            "engines", {}
        ):
    ...
    def init_from_real(self, real_usdc: float, real_sol: float):
        ...
        if engine := self.cm.state.get("engines", {}).get(self.engine_name):
            ...
            self.cm._save_state()
            Logger.info(
                f"💼 [PAPER:{self.engine_name}] State Reset: ${real_usdc:.2f} USDC | {real_sol:.3f} SOL | Positions Cleared"
            )
    ...
    def simulate_buy(
        ...
        if symbol not in engine_state["positions"]:
            engine_state["positions"][symbol] = PaperAsset(
                symbol=symbol, mint="", balance=0.0, avg_price=0.0
            )
        ...
        engine_state["stats"]["wins"] = engine_state["stats"].get(
            "wins", 0
        )  # Just to ensure key exists
