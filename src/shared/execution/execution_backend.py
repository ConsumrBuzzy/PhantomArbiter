...
class PaperBackend:
        ...
        **kwargs,
        ...
        if not engine_state:
            return TradeResult.failed(
                token, "BUY", f"Engine {self.engine_name} not found", "PAPER"
            )
        ...
        engine_state["stats"]["fees_paid_usd"] = (
            engine_state["stats"].get("fees_paid_usd", 0) + total_fees
        )
        ...
        if token not in engine_state["positions"]:
            engine_state["positions"][token] = PaperAsset(
                symbol=token, mint=mint, balance=0.0, avg_price=0.0
            )
    ...
    def execute_sell(
        ...
        **kwargs,
        ...
        if not engine_state:
            return TradeResult.failed(
                token, "SELL", f"Engine {self.engine_name} not found", "PAPER"
            )
        ...
        engine_state["stats"]["fees_paid_usd"] = (
            engine_state["stats"].get("fees_paid_usd", 0) + fees
        )
        engine_state["stats"]["total_pnl_usd"] = (
            engine_state["stats"].get("total_pnl_usd", 0) + net_pnl
        )
