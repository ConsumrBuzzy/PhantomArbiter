...
class UnifiedEngineAdapter:
    ...
    async def pre_check_arbitrage(
        self, mint_in: str, mint_out: str, amount: int, min_profit_bps: int = 10
    ) -> tuple[bool, int]:
        """
                ...
                if response.status_code == 200:
                    ...
                    if amount > 0:
                        ...
                        if profit_bps >= min_profit_bps:
                            Logger.debug(
                                f"[PRE-CHECK] ✅ Profit {profit_bps:.1f} bps >= {min_profit_bps} bps"
                            )
                    ...
                    Logger.warning(
                        f"[PRE-CHECK] Jupiter API returned {response.status_code}"
                    )
    ...
    async def execute_atomic_arb(
        ...
        try:
            ...
            is_atomic_pair = (buy_dex in ["meteora", "orca"]) and (
                sell_dex in ["meteora", "orca"]
            )

            if pre_check and not is_atomic_pair:
                Logger.info(
                    "[UNIFIED] 🔍 Running Jupiter pre-check (Aggregator Route)..."
                )
                ...
                if not is_profitable:
                    return AtomicArbResult(
                        success=False,
                        error="Pre-check failed: Jupiter quote unprofitable",
                        execution_time_ms=int((time.time() - start_time) * 1000),
                    )
                Logger.info(f"[UNIFIED] ✅ Pre-check passed (out: {jup_out})")
            elif is_atomic_pair:
                Logger.info(
                    f"[UNIFIED] ⏩ Skipping Jupiter pre-check for Atomic Route ({buy_dex}↔{sell_dex})"
                )
            ...
            if use_dynamic_fee:
                ...
                Logger.debug(
                    f"[UNIFIED] 💰 Priority fee ({priority_tier}): {priority_fee_micro} µL"
                )
            ...
            if jito_tip_lamports is None:
                ...
                estimated_profit_usd = (amount_in / 1_000_000) * (
                    opportunity.spread_pct / 100
                )

                from src.shared.execution.tip_optimizer import get_tip_optimizer

                optimizer = get_tip_optimizer()
                jito_tip_lamports = optimizer.get_optimized_tip(
                    opportunity.pair, estimated_profit_usd
                )

            # Simulate first (seatbelt)
            if simulate_first:
                Logger.info("[UNIFIED] 🧪 Running simulation...")
                sim_result = self._bridge.simulate(
                    legs, self._private_key, rpc_url=rpc_url
                )

                if not sim_result.simulation_success:
                    Logger.warning(
                        f"[UNIFIED] ❌ Simulation failed: {sim_result.simulation_error}"
                    )
                    return AtomicArbResult(
                        success=False,
                ...
                Logger.info(
                    f"[UNIFIED] ✅ Simulation passed (CU: {sim_result.compute_units_used})"
                )

            # Execute
            Logger.info(
                f"[UNIFIED] 🚀 Executing with {jito_tip_lamports} lamport Jito tip..."
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "win_rate": self.successful_arbs / self.total_arbs
            if self.total_arbs > 0
            else 0,
