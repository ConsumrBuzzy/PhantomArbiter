...
class PumpPortalAdapter:
    ...
    def __init__(self, private_key_b58: str = None):
        ...
        self.api_url = getattr(
            Settings, "PUMP_PORTAL_API_URL", "https://pumpportal.fun/api/trade-local"
        )
        ...
        try:
            ...
            Logger.info(
                f"💊 [PUMP] Adapter initialized for wallet: {self.pubkey_str[:8]}..."
            )
    ...
    def generate_transaction(
        ...
        try:
            ...
            if response.status_code != 200:
                Logger.error(
                    f"❌ [PUMP] API Error ({response.status_code}): {response.text}"
                )
                return None
    ...
    def execute_trade(self, action: str, mint: str, amount: float, **kwargs):
        ...
        Logger.info(
            f"💊 [PUMP] Generating {action.upper()} for {mint[:8]}... (Amt: {amount})"
        )
