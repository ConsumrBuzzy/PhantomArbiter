...
from src.shared.execution.schemas import calculate_arb_strategy, ArbTask, SwapLeg
...
class ArbDetector:
    ...
    async def start(self) -> bool:
        ...
        if not self._bridge.is_available():
            Logger.error("[ARB] Execution engine not available. Run: cd bridges && npm run build")
            return False
    ...
    async def _on_price_update(self, price: PoolPrice, pair: ArbPair):
        ...
        if opportunity and opportunity.is_viable:
            ...
            Logger.info(f"[ARB] 💰 Opportunity: {pair.name} | {opportunity.spread_bps}bps spread")
    ...
    async def _check_arb(self, pair: ArbPair) -> Optional[ArbOpportunity]:
        ...
        if spread_a_to_b > pair.min_profit_bps:
            ...
            strategy = calculate_arb_strategy(pair.trade_amount, pair.trade_amount + estimated_profit)
    ...
    async def _execute_arb(self, opp: ArbOpportunity, pair: ArbPair):
        ...
        try:
            ...
            # Build legs
            buy_pool_type = pair.pool_a_type if opp.direction == "A_to_B" else pair.pool_b_type
            sell_pool_type = pair.pool_b_type if opp.direction == "A_to_B" else pair.pool_a_type
