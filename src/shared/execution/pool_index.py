...
class PoolIndex:
        ...
        try:
            if db_manager:
                with db_manager.cursor() as c:
                    c.execute(
                        "SELECT symbol FROM pool_registry WHERE mint = ?", (mint,)
                    )
                    row = c.fetchone()
                    if row:
                        return row["symbol"]
    ...
    def _resolve_mint(self, symbol: str) -> Optional[str]:
        ...
        try:
            if db_manager:
                with db_manager.cursor() as c:
                    c.execute(
                        "SELECT mint FROM pool_registry WHERE symbol = ?", (symbol,)
                    )
                    row = c.fetchone()
                    if row:
                        return row["mint"]
    ...
    def _discover_pools(
        self, token_a: str, token_b: str, mint_a: str = None, mint_b: str = None
    ) -> Optional[PoolPair]:
        ...
        if (
            not meteora_pool
            and not orca_pool
            and not raydium_clmm_pool
            and not raydium_standard_pool
        ):
            return None
        ...
        Logger.debug(
            f"[POOL] Discovered {pair_name}: M={meteora_pool is not None}, O={orca_pool is not None}, R={raydium_clmm_pool or raydium_standard_pool is not None}"
        )
        return pool_pair
    ...
    def record_execution(
        ...
        try:
            ...
            if pair in self._pool_cache:
                ...
                if dex == "meteora":
                    if success:
                        pp.meteora_success_rate = self._calculate_success_rate(
                            pair, "meteora"
                        )
                elif dex == "orca":
                    if success:
                        pp.orca_success_rate = self._calculate_success_rate(
                            pair, "orca"
                        )
            ...
            if db_manager:
                with db_manager.cursor() as c:
                    ...
                    if rows:
                        best = max(
                            rows,
                            key=lambda r: r["successes"] / r["total"]
                            if r["total"] > 0
                            else 0,
                        )
                ...
                with db_manager.cursor() as c:
                    ...
                    for row in c.fetchall():
                        self._pool_cache[row["pair"]] = PoolPair(
                            ...
                            raydium_clmm_pool=row["raydium_clmm_pool"]
                            if "raydium_clmm_pool" in row.keys()
                            else None,
                            raydium_standard_pool=row["raydium_standard_pool"]
                            if "raydium_standard_pool" in row.keys()
                            else None,
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        raydium_count = sum(
            1
            for p in self._pool_cache.values()
            if p.raydium_clmm_pool or p.raydium_standard_pool
        )
        both_count = sum(
            1 for p in self._pool_cache.values() if p.meteora_pool and p.orca_pool
        )
    ...
    def can_use_unified_engine(self, opportunity) -> bool:
        ...
        buy_ok = (
            (buy_dex == "meteora" and pools.meteora_pool)
            or (buy_dex == "orca" and pools.orca_pool)
            or (
                buy_dex == "raydium"
                and (pools.raydium_clmm_pool or pools.raydium_standard_pool)
            )
        )
        sell_ok = (
            (sell_dex == "meteora" and pools.meteora_pool)
            or (sell_dex == "orca" and pools.orca_pool)
            or (
                sell_dex == "raydium"
                and (pools.raydium_clmm_pool or pools.raydium_standard_pool)
            )
