    ...
    def get_optimized_tip(self, pair: str, expected_profit_usd: float) -> int:
        ...
        if stats["total"] >= 3:
            if stats["inclusion_rate"] < 0.6:
                ...
                Logger.info(
                    f"📈 [TIP] inclusion low ({stats['inclusion_rate'] * 100:.0f}%), bumping mult: {new_mult:.2f}"
                )
        ...
        if final_tip > self.min_tip_lamports:
            Logger.debug(
                f"[TIP] Optimized tip for {pair}: {final_tip} lamps ({new_mult * 100:.1f}% of ${expected_profit_usd:.2f})"
            )
