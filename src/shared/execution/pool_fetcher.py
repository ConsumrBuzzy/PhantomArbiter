...
class MeteoraPoolFetcher:
                ...
                try:
                    self._cache.append(
                        MeteoraPool(
                            ...
                            base_fee_bps=float(pool.get("base_fee_percentage", 0))
                            * 100,
                        )
                    )
    ...
    def get_all_pools(
        self, min_liquidity: float = 0, force_refresh: bool = False
    ) -> List[MeteoraPool]:
        """
    ...
    def get_pools_for_token(
        self, token_symbol: str, min_liquidity: float = 10000
    ) -> List[MeteoraPool]:
        """
        ...
        pools = [
            ...
            if (
                token_upper in p.name.upper()
                or token_upper in p.token_x_symbol.upper()
                or token_upper in p.token_y_symbol.upper()
            )
    ...
    def get_best_pool(
        self, token_a: str, token_b: str, min_liquidity: float = 5000
...
if __name__ == "__main__":
    ...
    for i, pool in enumerate(sol_pools[:10], 1):
        print(
            f"  {i}. {pool.name:<20} | TVL: ${pool.liquidity:>12,.0f} | Bin: {pool.bin_step:>3}"
        )
