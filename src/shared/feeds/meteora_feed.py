...
class MeteoraFeed(PriceSource):
        ...
        try:
            ...
            try:
                ...
                if pools and pools.meteora_pool:
                    ...
                    if result:
                        Logger.debug(
                            f"[METEORA] Bridge Result: success={result.success}, err={result.error}"
                        )
                    ...
                    if result and result.success:
                        ...
                        if price > 0:
                            Logger.debug(
                                f"[METEORA] 🟢 Daemon price for {base_mint[:4]}: ${price}"
                            )
                        ...
                        Logger.debug(
                            f"[METEORA] Daemon price failed: {result.error if result else 'No result'}"
                        )
            except Exception as e:
                Logger.debug(f"[METEORA] Daemon check failed: {e}")

            # 2. Fetch from DexScreener (Fallback)
            result = await self._fetch_dexscreener_price(
                base_mint, dex_filter="meteora"
            )
    ...
    async def _fetch_dexscreener_price(
        ...
        try:
            ...
            if dex_filter:
                filtered = [
                    p for p in pairs if dex_filter.lower() in p.get("dexId", "").lower()
                ]
                if filtered:
                    pairs = filtered
                else:
    ...
    async def get_multiple_prices(
        self, mints: list, vs_token: str = None
    ) -> Dict[str, float]:
        """
                    ...
                    if resp.status_code == 200:
                        ...
                        for pair in pairs:
                            ...
                            if base and price > 0 and base not in prices:
                                prices[base] = price
                                # Cache
                                self._price_cache[f"{base}_{self.USDC_MINT}"] = (
                                    price,
                                    time.time(),
                                )
                except Exception as e:
...
if __name__ == "__main__":
    ...
    if spot:
        print(
            f"✅ BONK/USDC: ${spot.price:.8f} (Liquidity: ${spot.liquidity_usd:,.0f})"
        )
    else:
