...
class Pod:
    ...
    smart_sizing_multiplier: float = (
        1.0  # V28: Smart Sizing (0.5x for risk, 2.0x for conviction)
    )
...
class PodBuilder:
    ...
    @staticmethod
    def from_named_list(
        name: str, token_list: List[Tuple[str, str]], tags: List[str] = None
    ) -> Pod:
        # Auto-size based on tags
        multiplier = 1.0
        if tags:
            if "MEME" in tags or "VOLATILE" in tags:
                multiplier = 0.5
            if "STABLE" in tags or "BLUE_CHIP" in tags:
                multiplier = 1.5
        ...
        return Pod(
            ...
            tokens=token_list,
            tags=tags or ["STATIC"],
            smart_sizing_multiplier=multiplier,
        )

    @staticmethod
    def create_smart_money_pod(
        name: str, tokens: List[Dict], min_score: float = 0.8
    ) -> Pod:
        ...
        return Pod(
            name=name, tokens=valid_tokens, priority=1, tags=["SMART_MONEY", "DYNAMIC"]
        )
...
class PodManager:
    ...
    def _init_defaults(self):
        ...
        for name, tokens in ALL_NAMED_PODS.items():
            # Heuristic tagging
            tags = ["STATIC"]
            if "OG" in name or "VIRAL" in name:
                tags.append("MEME")
            if "DEFI" in name:
                tags.append("STABLE")
            if "AI" in name:
    ...
    def inject_priority_token(
        self, mint: str, symbol: str = "AUTO", tag: str = "PRIORITY"
    ):
