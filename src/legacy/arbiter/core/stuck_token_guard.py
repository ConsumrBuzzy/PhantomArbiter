...
class StuckTokenGuard:
    ...
    def __init__(self):
        ...
        self.rpc_url = os.getenv(
            "RPC_URL", Settings.RPC_URLS[0] if hasattr(Settings, "RPC_URLS") else ""
        )
    ...
    def check_wallet(self) -> List[StuckToken]:
        ...
        try:
            ...
            for account in accounts:
                ...
                Logger.warning(
                    f"[GUARD] 🚨 STUCK TOKEN: {balance:.4f} {symbol} (${value_usd:.2f})"
                )
    ...
    def attempt_recovery(self, stuck_tokens: List[StuckToken]) -> Dict[str, bool]:
        ...
        for token in stuck_tokens:
            if token.value_usd < self.MIN_VALUE_TO_RECOVER:
                Logger.info(
                    f"[GUARD] Skipping {token.symbol} (value ${token.value_usd:.2f} < min ${self.MIN_VALUE_TO_RECOVER})"
                )
                results[token.mint] = False
                continue

            Logger.info(
                f"[GUARD] 🔄 Attempting recovery: {token.balance:.4f} {token.symbol} → USDC"
            )
            ...
            try:
                ...
                quote = self.router.get_jupiter_quote(
                    token.mint, USDC_MINT, amount, slippage_bps=self.SLIPPAGE_BPS
                )
                ...
                Logger.info(
                    f"[GUARD] Quote: {token.balance:.4f} {token.symbol} → ${expected_usdc:.2f} USDC"
                )
                ...
                try:
                    ...
                    if not swap_data or "swapTransaction" not in swap_data:
                        Logger.warning(
                            f"[GUARD] Could not get swap tx for {token.symbol}"
                        )
                        results[token.mint] = False
                        continue
                    ...
                    if send_result.status_code == 200:
                        ...
                        if "result" in result_json:
                            ...
                            Logger.info(
                                f"[GUARD] ✅ Recovery SUCCESS: {token.symbol} → USDC | Tx: {sig[:20]}..."
                            )
                            print(
                                f"   ✅ [GUARD] Recovered {token.symbol} → ${expected_usdc:.2f} USDC"
                            )
