...
class OpportunityVerifier:
    ...
    async def verify(
        self, opportunities: List[SpreadOpportunity], trade_size: float
        ...
        candidates = sorted(candidates, key=lambda x: x.net_profit_usd, reverse=True)[
            : self.batch_size
        ]
        ...
        async def verify_one(opp: SpreadOpportunity) -> SpreadOpportunity:
            ...
            is_valid, real_net, status = await self.executor.verify_liquidity(
                opp, trade_size
            )
