...
class ArbitrageRiskManager:
    ...
    def validate_opportunity(self, opportunity) -> Tuple[bool, str]:
        ...
        if opportunity.net_profit_usd < self.min_profit_usd:
            return (
                False,
                f"Net profit ${opportunity.net_profit_usd:.2f} < min ${self.min_profit_usd:.2f}",
            )
        ...
        if opportunity.spread_pct < self.max_slippage_pct:
            return (
                False,
                f"Spread {opportunity.spread_pct:.2f}% may be eaten by slippage",
            )
    ...
    def calculate_safe_size(self, opportunity, available_balance: float) -> float:
        """
        ...
        )  # Never use more than 50% in one trade
