...
class RebalancingEngine:
    ...
    async def check_and_rebalance(self, position) -> Optional[RebalanceAction]:
        ...
        Logger.debug(
            f"[REBAL] {position.coin}: Spot {spot_weight * 100:.1f}% / Perp {perp_weight * 100:.1f}% | Drift: {delta_drift * 100:.1f}%"
        )
        ...
        if abs(imbalance_usd) < self.min_rebalance_usd:
            Logger.debug(
                f"[REBAL] Imbalance ${imbalance_usd:.2f} below min ${self.min_rebalance_usd}"
            )
            return None
        ...
        if spot_weight > target_weight:
            ...
            action = RebalanceAction(
                ...
                current_delta=spot_weight / perp_weight
                if perp_weight > 0
                else float("inf"),
            )
        else:
    ...
    async def execute_rebalance(self, action: RebalanceAction) -> Dict[str, Any]:
        ...
        Logger.info(
            f"[REBAL] Executing {action.action} for ${action.amount_usd:.2f}..."
        )
        ...
        Logger.info("[REBAL] ✅ Rebalance complete (simulated)")
    ...
    async def run_loop(self, auto_execute: bool = False):
        ...
        self._running = True
        Logger.info(
            f"[REBAL] Starting rebalancing loop (interval: {self.check_interval // 60} min)"
        )
        ...
        while self._running:
            try:
                ...
                if actions:
                    ...
                    Logger.debug("[REBAL] All positions within tolerance")
...
if __name__ == "__main__":
    ...
    async def test():
        ...
        rebalancer = RebalancingEngine(
            atomic_executor=executor, check_interval=60, max_delta_drift=0.05
        )
