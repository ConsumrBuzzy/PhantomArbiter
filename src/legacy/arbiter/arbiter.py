...
class PhantomArbiter:
    ...
    def __init__(self, config: ArbiterConfig):
        ...
        self.tracker = (
            None  # Deprecated, kept for interface compatibility if needed transiently
        )
    ...
    def _setup_paper_mode(self) -> None:
        ...
        self._executor = ArbitrageExecutor(mode=mode, wallet=self.paper_wallet)
    ...
    async def _setup_live_mode_async(self) -> None:
        ...
        private_key = os.getenv("PHANTOM_PRIVATE_KEY") or os.getenv(
            "SOLANA_PRIVATE_KEY"
        )
        ...
        try:
            ...
            if self.config.use_unified_engine:
                ...
                if self._unified_adapter.is_available():
                    ...
                    Logger.warning(
                        "⚡ Unified Engine: NOT AVAILABLE (run: cd bridges && npm run build)"
                    )
                    self._unified_adapter = None
            ...
            if self.config.full_wallet:
                ...
                if not getattr(Settings, "SILENT_MODE", False):
                    print(
                        f"   ✅ LIVE MODE ENABLED - Wallet: {self._wallet.get_public_key()[:8]}..."
                    )
                    print(
                        f"   💰 USDC: ${self.current_balance:.2f} | SOL (Gas): ${self.gas_balance:.2f}"
                    )
                Logger.info(
                    f"✅ LIVE MODE ENABLED - Wallet: {self._wallet.get_public_key()[:8]}..."
                )
            ...
            self.trade_engine = TradeEngine(
                ...
                use_unified=self.config.use_unified_engine
                and (self._unified_adapter is not None),
            )
    ...
    async def scan_opportunities(
        self, verbose: bool = True, scanner: Optional[AdaptiveScanner] = None
        ...
        if self._triangular_scanner:
            try:
                ...
                if cycles:
                    top_cycles = sorted(
                        cycles, key=lambda x: x.net_profit_usd, reverse=True
                    )[:2]
    ...
    async def execute_trade(
        self, opportunity: SpreadOpportunity, trade_size: float = None
    ) -> Dict[str, Any]:
    ...
    def _check_signals(self):
        ...
        try:
            ...
            for symbol, score in hot_tokens.items():
                ...
                Logger.info(
                    f"   🧠 SIGNAL: Added {symbol} (Trust: {score:.1f}) to Arbiter scan list"
                )

            if added_count > 0:
                if not getattr(Settings, "SILENT_MODE", False):
                    print(
                        f"   🧠 Scraper Signal: Added {added_count} hot tokens to scan list."
                    )
        ...
        try:
            ...
            if self._engine:
                ...
                if hasattr(self._engine, "inject_priority_token"):
                    ...
                    Logger.info(
                        f"⚡ [ARB] Received Strategy Tip: {token_symbol} (Engine notified)"
                    )
                    ...
                    if mint:
                        ...
                        if new_pair not in self.config.pairs:
                            self.config.pairs.append(new_pair)
                            Logger.info(
                                f"   ➕ Added {token_symbol} to immediate scan list."
                            )
            else:
                Logger.warning(
                    f"⚠️ [ARB] Received tip for {token_symbol} but Engine is offline."
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--interval", type=int, default=5, help="Scan interval in seconds"
    )
    parser.add_argument(
        "--min-spread", type=float, default=0.50, help="Minimum spread percent"
    )
    parser.add_argument(
        "--max-trade", type=float, default=10.0, help="Maximum trade size"
    )
    parser.add_argument(
        "--full-wallet", action="store_true", help="Use entire wallet balance"
    )
