...
class LongtailLogic:
    ...
    def analyze_tick(self, watcher: Watcher, price: float) -> tuple:
        ...
        if hasattr(watcher, "last_signal_time") and (
            time.time() - watcher.last_signal_time < 60
        ):
            return "HOLD", "", 0.0
        ...
        prev_state = self._macd_cache.get(
            symbol, {"macd": 0, "signal": 0, "histogram": 0}
        )

        # Update cache
        self._macd_cache[symbol] = {
            "macd": macd,
            "signal": signal,
            "histogram": histogram,
        }
        ...
        if watcher.in_position:
            return self._evaluate_exit(
                watcher, price, macd, signal, histogram, prev_state
            )
    ...
    def _evaluate_entry(
        self, watcher, price, macd, signal, histogram, prev_state
    ) -> tuple:
        ...
        if prev_hist <= 0 and histogram > 0:
            ...
            reason = (
                f"🔭 LONGTAIL MACD CROSS (MACD:{macd:.6f} > SIG:{signal:.6f}) [{info}]"
            )
            priority_queue.add(
                3,
                "LOG",
                {
                    "level": "INFO",
                    "message": f"[LONGTAIL] 📈 MACD Bullish Cross: {watcher.symbol}",
                },
            )
    ...
    def _evaluate_exit(
        ...
        if triggered:
            net_pnl = (
                Metrics.calculate_net_pnl_pct(
                    watcher.entry_price, price, watcher.cost_basis
                )
                * 100
            )
