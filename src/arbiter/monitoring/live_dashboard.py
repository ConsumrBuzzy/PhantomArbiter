...
class LiveDashboard:
    ...
    def _center(self, text: str, width: int) -> str:
        ...
        text_len = len(text.replace('\033[0m', '').replace('\033[1m', '')
                       .replace('\033[2m', '').replace('\033[91m', '')
                       .replace('\033[92m', '').replace('\033[93m', '')
                       .replace('\033[94m', '').replace('\033[95m', '')
    ...
    def _pad_cell(self, text: str, width: int) -> str:
        ...
        for code in ['\033[0m', '\033[1m', '\033[2m', '\033[91m', '\033[92m', 
                     '\033[93m', '\033[94m', '\033[95m', '\033[96m', '\033[97m', '\033[90m']:
    ...
    def generate_dashboard(self) -> str:
        ...
        # Header
        lines.append(f"{self.CYAN}{self._make_box_line(self.BOX['tl'], self.BOX['h'], self.BOX['tr'], WIDTH)}{self.RESET}")
        lines.append(f"{self.CYAN}{self.BOX['v']}{self.RESET}{self._center(f'{self.BOLD}ARBITRAGE ENGINE v1.0{self.RESET}', WIDTH-2)}{self.CYAN}{self.BOX['v']}{self.RESET}")
        lines.append(f"{self.CYAN}{self.BOX['v']}{self.RESET}{self._center(f'Mode: {self.YELLOW}{self.mode}{self.RESET} | Budget: {self.GREEN}${self.stats.budget:.0f}{self.RESET}', WIDTH-2)}{self.CYAN}{self.BOX['v']}{self.RESET}")
        lines.append(f"{self.CYAN}{self._make_box_line(self.BOX['lm'], self.BOX['h'], self.BOX['rm'], WIDTH)}{self.RESET}")
        
        # Live Spreads Section
        now = datetime.now().strftime('%H:%M:%S')
        lines.append(f"{self.CYAN}{self.BOX['v']}{self.RESET} {self.BOLD}LIVE SPREADS{self.RESET}                                      Updated: {self.DIM}{now}{self.RESET} {self.CYAN}{self.BOX['v']}{self.RESET}")
        lines.append(f"{self.CYAN}{self._make_box_line(self.BOX['lm'], self.BOX['h'], self.BOX['rm'], WIDTH)}{self.RESET}")
        ...
        lines.append(header)
        lines.append(f"{self.CYAN}{self.BOX['v']}{self.RESET}{'─'*11}┼{'─'*11}┼{'─'*11}┼{'─'*10}┼{'─'*14}┼{'─'*12}{self.CYAN}{self.BOX['v']}{self.RESET}")
        ...
        if self.spreads:
            ...
            lines.append(f"{self.CYAN}{self.BOX['v']}{self.RESET} {self.DIM}Waiting for price data...{self.RESET}{' '*43}{self.CYAN}{self.BOX['v']}{self.RESET}")
        
        # Funding Rates Section (if enabled)
        if self.mode in ['FUNDING', 'ALL']:
            lines.append(f"{self.CYAN}{self._make_box_line(self.BOX['lm'], self.BOX['h'], self.BOX['rm'], WIDTH)}{self.RESET}")
            lines.append(f"{self.CYAN}{self.BOX['v']}{self.RESET} {self.BOLD}FUNDING RATES (Drift){self.RESET}{' '*48}{self.CYAN}{self.BOX['v']}{self.RESET}")
            lines.append(f"{self.CYAN}{self._make_box_line(self.BOX['lm'], self.BOX['h'], self.BOX['rm'], WIDTH)}{self.RESET}")
            ...
            if self.funding_rates:
                for market, rate in list(self.funding_rates.items())[:3]:
                    ...
                    status = f"{self.GREEN}PROFITABLE{self.RESET}" if abs(rate) >= min_rate else f"{self.DIM}BELOW MIN{self.RESET}"
                    lines.append(f"{self.CYAN}{self.BOX['v']}{self.RESET} {market:<10} │ {rate_str}/8h │ Next: 2h 15m │ {status}{' '*16}{self.CYAN}{self.BOX['v']}{self.RESET}")
            else:
                lines.append(f"{self.CYAN}{self.BOX['v']}{self.RESET} {self.DIM}Connecting to Drift...{self.RESET}{' '*47}{self.CYAN}{self.BOX['v']}{self.RESET}")
        
        # Today's Performance
        lines.append(f"{self.CYAN}{self._make_box_line(self.BOX['lm'], self.BOX['h'], self.BOX['rm'], WIDTH)}{self.RESET}")
        lines.append(f"{self.CYAN}{self.BOX['v']}{self.RESET} {self.BOLD}TODAY'S PERFORMANCE{self.RESET}{' '*50}{self.CYAN}{self.BOX['v']}{self.RESET}")
        lines.append(f"{self.CYAN}{self._make_box_line(self.BOX['lm'], self.BOX['h'], self.BOX['rm'], WIDTH)}{self.RESET}")
        ...
        if profit >= 0:
            ...
            profit_str = f"{self.RED}-${abs(profit):.2f} ({profit_pct:.2f}%){self.RESET}"
        
        perf_line = f" Turnover: {self.YELLOW}{turnover:.1f}x{self.RESET} │ Trades: {trades} │ Volume: ${volume:,.2f} │ Profit: {profit_str}"
        lines.append(f"{self.CYAN}{self.BOX['v']}{self.RESET}{perf_line}{' '*(WIDTH - len(perf_line.replace(self.RESET, '').replace(self.GREEN, '').replace(self.YELLOW, '').replace(self.RED, '')) - 3)}{self.CYAN}{self.BOX['v']}{self.RESET}")
        
        # Footer
        lines.append(f"{self.CYAN}{self._make_box_line(self.BOX['bl'], self.BOX['h'], self.BOX['br'], WIDTH)}{self.RESET}")
    ...
    def get_telegram_summary(self) -> str:
        ...
        for spread in self.spreads[:3]:
            emoji = "🟢" if spread.status == "READY" else "🔵" if spread.status == "MONITOR" else "⚪"
