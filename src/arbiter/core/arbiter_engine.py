...
class ArbiterEngine:
    ...
    async def run(
        ...
        signal_config = CoordinatorConfig(
            wss_enabled=adaptive_mode, pairs=self.config.pairs, scraper_poll_interval=60
        )
        coordinator = SignalCoordinator(
            signal_config, on_activity, on_flash_warm=on_flash_warm
        )
            ...
            while time.time() < end_time:
                ...
                try:
                    ...
                    opportunities, all_spreads = await self.arbiter.scan_opportunities(
                        verbose=should_print, scanner=monitor if adaptive_mode else None
                    )
                    ...
                    current_spreads = (
                        {opp.pair: (opp.spread_pct, time.time()) for opp in all_spreads}
                        if all_spreads
                        else {}
                    )
                    ...
                    try:
                        ...
                        for pair, (spread, ts) in current_spreads.items():
                            if pair in self._last_spreads:
                                ...
                                if (
                                    time_delta > 0 and prev_spread > 0.1
                                ):  # Only log meaningful spreads
                                    db_manager.log_spread_decay(
                                        pair, prev_spread, spread, time_delta
                                    )
                    except Exception:
                    ...
                    app_state.opportunities = [
                        ArbOpportunity(
                            ...
                            est_profit_sol=o.net_profit_usd
                            / 150.0,  # Mock conversion for Sol
                        )
                        for o in sorted(
                ...
                # 4. VERIFICATION
                verified_opps = await self._verify_top_candidates(
                ...
                profitable_count = len(
                    [o for o in opportunities if o.net_profit_usd > 0]
                )
                ...
                executed_this_cycle = await self._process_executions(
                    opportunities,
                    verified_opps,
                    base_trade_size,
                    last_trade_time,
                ...
                if landlord and self.config.live_mode and not executed_this_cycle:
                    await landlord.tick(
                        self.arbiter.current_balance, arb_opportunity=False
                    )
    ...
    async def _perform_maintenance(self, coordinator, now):
        ...
        if len(self.config.pairs) > 60:
            try:
                ...
                sorted_pairs = sorted(
                    self.config.pairs,
                    key=lambda p: pair_scores.get(p[0], 0.5),
                    reverse=True,
                )
    ...
    def _calculate_trade_size(self, pair: str = None) -> float:
        ...
        ):  # Assuming self.config.live_mode exists, otherwise self.live_mode
            # V2.0: Use Arbiter's unified property (Live or Paper)
            base = min(self.arbiter.current_balance, limit)
        else:
            # Paper mode
            base = min(
                self.arbiter.current_balance, limit
            )  # Corrected: min returns a number, not an object with get_smart_size_multiplier
    ...
    async def _verify_top_candidates(
        self, opportunities, trade_size, last_trade_time, cooldown
    ):
        ...
        async def verify_one(opp):
            is_valid, real_net, status = await self.arbiter._executor.verify_liquidity(
                opp, trade_size
            )
    ...
    async def _process_executions(
        self, opportunities, verified_opps, trade_size, last_trade_time, cooldown
    ) -> bool:
        ...
        for op in sorted(opportunities, key=lambda x: x.net_profit_usd, reverse=True):
            ...
            if self.arbiter.trade_engine.use_unified:
                if op.buy_dex in ["METEORA", "ORCA"] and op.sell_dex in [
                    "METEORA",
                    "ORCA",
                ]:
            ...
            pair_threshold = get_pair_threshold(
                op.pair, self.config.fast_path_threshold
            )
            ...
            if (
                op.net_profit_usd - self.config.decay_buffer
            ) > 0.10 and op.net_profit_usd >= pair_threshold:
        ...
        valid_opps = [
            op for op in verified_opps if "LIVE" in str(op.verification_status or "")
        ]
        if valid_opps:
