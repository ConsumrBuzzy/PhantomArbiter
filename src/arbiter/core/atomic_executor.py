...
class AtomicExecutor:
    ...
    def __init__(
        self, wallet=None, jupiter=None, drift=None, jito=None, paper_mode: bool = True
    ):
        self.wallet = wallet
    ...
    async def execute_funding_arb(
        self, target_coin: str, amount_usd: float, leverage: float = 1.0
    ) -> Dict[str, Any]:
        """
        ...
        Logger.info(
            f"         Spot: ${spot_amount_usd:.2f} | Perp Short: ${perp_amount_usd:.2f}"
        )
        ...
        try:
            if self.paper_mode:
                ...
                return await self._paper_entry(
                    target_coin, spot_amount_usd, perp_amount_usd
                )
            else:
                # ═══ LIVE MODE ═══
    ...
    async def _paper_entry(
        self, coin: str, spot_usd: float, perp_usd: float
    ) -> Dict[str, Any]:
        ...
        Logger.info("[ATOMIC] ✅ Paper entry complete!")
        Logger.info(f"         Spot: {spot_amount:.6f} {coin} @ ${spot_price:.2f}")
        Logger.info(
            f"         Perp: -{perp_amount:.6f} {coin}-PERP @ ${perp_price:.2f}"
        )
    ...
    async def _get_jupiter_instructions(
        self, coin: str, amount_usd: float
    ) -> Optional[Any]:
    ...
    async def _get_drift_short_instructions(
        self, coin: str, amount_usd: float, leverage: float = 1.0
    ) -> Optional[Any]:
    ...
    async def close_position(self, coin: str) -> Dict[str, Any]:
        ...
        if self.paper_mode:
            ...
            perp_pnl = abs(position.perp_amount) * (
                position.entry_perp_price - perp_price
            ...
            Logger.info("[ATOMIC] ✅ Paper exit complete!")
