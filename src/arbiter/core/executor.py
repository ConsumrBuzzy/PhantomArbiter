...
class ArbitrageExecutor:
    ...
    async def _fetch_quotes_async(
        self, opportunity, usdc_amount: int, slippage_bps: int = 50
    ...
    def _calculate_optimal_size(
        self, failed_size: float, failed_net: float, opportunity, real_slippage: float
    ) -> float:
        """
        ...
        try:
            ...
            expected_net = (
                failed_size * (opportunity.spread_pct / 100) - 0.01
            )  # approx fee
            ...
            if k <= 0:
                return 10.0  # Should not happen if loss occurred
    ...
    async def verify_liquidity(
        self, opportunity, trade_size: float = None
        ...
        try:
            ...
            opt_size = self._calculate_optimal_size(
                trade_size, real_net, opportunity, 0
            )

            if opt_size < trade_size and opt_size >= 10.0:
                Logger.info(
                    f"   📉 Adapting Size: ${trade_size:.2f} -> ${opt_size:.2f} (Calculated Optimal)"
                )

                # Verify optimal size
                is_valid, new_net, status = await self.verify_liquidity(
                    opportunity, opt_size
                )
                if is_valid:
    ...
    async def execute_spatial_arb(
        self, opportunity, trade_size: float = None, rpc: Any = None
    ) -> ArbitrageExecution:
        """
        ...
        try:
            ...
            if not getattr(opportunity, "buy_quote", None) or not getattr(
                opportunity, "sell_quote", None
            ):
                Logger.info(
                    f"[EXEC] 🚀 Quote Cache Miss (FAST PATH) - Fetching for {opportunity.pair}"
                )
                usdc_amount = int(trade_size * 1_000_000)
            ...
            if scan_spread_pct >= SKIP_QUOTE_THRESHOLD:
                ...
                if projected_profit_usd < MIN_SKIP_QUOTE_PROFIT:
                    Logger.info(
                        f"[EXEC] ⏭️ Skip-quote trade too thin: Net ${projected_profit_usd:.3f} < ${MIN_SKIP_QUOTE_PROFIT}"
                    )
                    return self._error_result(
                        f"Skip-quote trade too thin: ${projected_profit_usd:.3f} < ${MIN_SKIP_QUOTE_PROFIT}",
                        start_time,
                    )
            else:
                ...
                if projected_profit_usd < MIN_NET_PROFIT_USD:
                    Logger.info(
                        f"[EXEC] ⏭️ Skipping thin spread: Net ${projected_profit_usd:.3f} < ${MIN_NET_PROFIT_USD}"
                    )
                    return self._error_result(
                        f"Net too thin: ${projected_profit_usd:.3f} < ${MIN_NET_PROFIT_USD}",
                        start_time,
                    )
            ...
            if self.mode == ExecutionMode.PAPER:
                ...
                if has_paper_wallet:
                    ...
                    if not buy_success:
                        return self._error_result(
                            "Paper Buy Failed (Insufficient API/Cash)", start_time
                        )
                ...
                legs = [
                    TradeResult(
                        ...
                        price=opportunity.buy_price,
                        fee_usd=buy_fee,
                        signature="PAPER_BUY_" + str(int(time.time())),
                        timestamp=time.time(),
                    ...
                    TradeResult(
                        ...
                        output_token="USDC",
                        input_amount=int(buy_quote.get("outAmount", 0))
                        / 1e9,  # Assuming full sell
                        output_amount=expected_usdc_back / 1_000_000,
                        price=opportunity.sell_price,
                        fee_usd=sell_fee,
                        signature="PAPER_SELL_" + str(int(time.time()) + 1),
                        timestamp=time.time(),
                        execution_time_ms=250,
                    ),
                ]
                ...
                jito_status = (
                    "READY"
                    if self.jito and await self.jito.is_available()
                    else ("MISSING" if not self.jito else "OFFLINE")
                )

                if jito_status == "READY":
                    Logger.info(
                        f"[EXEC] 🛡️ Using Jito atomic bundle (Elite RPC: {getattr(rpc, 'name', 'Winner')})..."
                    )
                    result = await self._execute_bundled_swaps(
                        buy_quote, sell_quote, rpc=rpc
                    )
                    ...
                    if result and result.get("success"):
                        ...
                        error_msg = (
                            result.get("error", "Jito bundle failed")
                            if result
                            else "Jito bundle execution failed"
                        )
                        Logger.warning(
                            f"[EXEC] Jito failed: {error_msg} - Trying sequential fallback..."
                        )
                        print(f"   ⚠️ Jito failed: {error_msg}")
                        print("   🔄 Attempting sequential RPC fallback...")

                        try:
                            # Sequential fallback: Execute buy, then sell
                            fallback_result = await self._execute_sequential_fallback(
                                buy_quote, sell_quote, rpc
                            )
                            if fallback_result and fallback_result.get("success"):
                                ...
                                fb_error = (
                                    fallback_result.get("error", "Unknown")
                                    if fallback_result
                                    else "Failed"
                                )
                                Logger.error(
                                    f"[EXEC] Sequential fallback also failed: {fb_error}"
                                )
                                return self._error_result(
                                    f"Both Jito and fallback failed: {fb_error}",
                                    start_time,
                                )
                        except Exception as e:
                            Logger.error(f"[EXEC] Sequential fallback exception: {e}")
                            return self._error_result(
                                f"Fallback exception: {e}", start_time
                            )
                else:
                    # V131: Jito OFFLINE - Try sequential fallback instead of aborting
                    Logger.warning(
                        f"[EXEC] 🛑 Jito {jito_status} - Attempting sequential fallback..."
                    )
                    print(
                        f"   ⚠️ Jito {jito_status} - Attempting sequential RPC fallback..."
                    )
                    ...
                    try:
                        ...
                        if fallback_result and fallback_result.get("success"):
                            Logger.info(
                                "[EXEC] ✅ Sequential fallback succeeded (Jito was offline)!"
                            )
                            print("   ✅ Sequential fallback SUCCEEDED!")
                            legs = fallback_result.get("legs", [])
                        else:
                            ...
                            Logger.error(
                                f"[EXEC] Sequential fallback failed: {fb_error}"
                            )
                            return self._error_result(
                                f"Fallback failed: {fb_error}", start_time
                            )
                    except Exception as e:
                        Logger.error(f"[EXEC] Sequential fallback exception: {e}")
                        return self._error_result(
    ...
    async def _execute_bundled_swaps(
        self, buy_quote: Dict, sell_quote: Dict, rpc: Any = None
    ) -> Dict:
        """
        ...
        try:
            Logger.info(
                "[EXEC] Building atomic bundle (V131: parallel tx + embedded tip)..."
            )
            ...
            if not tip_account:
                ...
                return {
                    "success": False,
                    "error": "No Jito tip account available",
                    "legs": [],
                }
            ...
            try:
                ...
                if sell_instructions:
                    ...
                    tipped_sell_tx = VersionedTransaction(
                        sell_msg, [self.wallet.keypair]
                    )
            ...
            if not sell_b58:
                ...
                Logger.info("[EXEC] 🚀 Submitting 3-tx fallback bundle...")
            else:
                # 2-tx optimized bundle
                tx_bundle = [buy_b58, sell_b58]
                Logger.info("[EXEC] 🚀 Submitting 2-tx bundle (V130 embedded tip)...")
            ...
            if not bundle_id:
                ...
                Logger.warning(
                    "[EXEC] ⚠️ Jito submission failed - Check simulation logs above"
                )
                Logger.debug(
                    f"[EXEC] Bundle txs: {[tx[:20] + '...' for tx in tx_bundle]}"
                )
                return {
                    "success": False,
            ...
            if not is_confirmed:
                ...
                Logger.warning("[EXEC] ⚠️ Jito bundle failed (Invalid/Dropped).")
                return {
                    "success": False,
                    "error": "Jito bundle failed verification",
                    "legs": [],
                    "should_fallback": True,
                }
            ...
            legs = [
                TradeResult(
                    ...
                    fee_usd=0.01,
                    signature=bundle_id,
                    error=None if is_confirmed else "Bundle failed to land",
                    timestamp=time.time(),
                    execution_time_ms=0,
                ),
                TradeResult(
    ...
    async def _execute_sequential_fallback(
        ...
        try:
            ...
            buy_sig = await self._submit_standard_tx(
                buy_tx_data["swapTransaction"], rpc
            )
            if not buy_sig:
                return {
                    "success": False,
                    "error": "Buy tx submission failed",
                    "legs": [],
                }
            ...
            fresh_sell_quote = router.get_jupiter_quote(
                ...
                output_mint,
                expected_tokens,
                slippage_bps=100,  # Wider slippage for safety
            )

            if fresh_sell_quote:
                Logger.debug("[EXEC] Using fresh sell quote (blockhash refreshed)")
            ...
            if not sell_tx_data or "swapTransaction" not in sell_tx_data:
                ...
                return {
                    "success": False,
                    "error": "Failed to get sell tx (recovery triggered)",
                    "legs": legs,
                }

            sell_sig = await self._submit_standard_tx(
                sell_tx_data["swapTransaction"], rpc
            )
            if not sell_sig:
                Logger.error(
                    "[EXEC] ⚠️ SELL SUBMISSION FAILED - Triggering stuck token recovery!"
                )
                print("   ⚠️ SELL SUBMISSION FAILED - Triggering stuck token guard...")
                self._trigger_stuck_token_recovery(buy_quote.get("outputMint", ""))
                return {
                    "success": False,
                    "error": "Sell tx submission failed (recovery triggered)",
                    "legs": legs,
                }
    ...
    def _trigger_stuck_token_recovery(self, mint: str):
        ...
        try:
            if self.stuck_token_guard:
                Logger.info(
                    f"[EXEC] 🔄 Triggering stuck token recovery for {mint[:8]}..."
                )
                # Run check immediately (synchronous)
                stuck_count = self.stuck_token_guard.run_check()
                Logger.info(
                    f"[EXEC] Stuck token check complete: {stuck_count} tokens processed"
                )
            else:
                Logger.warning(
                    "[EXEC] No stuck token guard available - manual recovery may be needed"
                )
        except Exception as e:
        ...
        try:
            ...
            if not result or not isinstance(result, dict) or not result.get("success"):
                error_msg = (
                    ...
                    else f"Unexpected return: {type(result)}"
                )
                return TradeResult(
                    success=False,
                    trade_type="SWAP",
            ...
            return TradeResult(
                ...
                output_amount=int(result.get("outAmount", quote.get("outAmount", 0)))
                / 1e6,
    ...
    async def execute_triangular_arb(
        self, opportunity, dry_run: bool = True, rpc: Any = None
    ) -> Optional[Dict]:
        """
        ...
        Logger.info(
            f"[EXEC] 📐 {'Observing' if dry_run else 'Executing'} Triangular: {' -> '.join(opportunity.route_tokens)}"
        )
        ...
        try:
            ...
            Logger.info(
                f"[EXEC] 📐 REAL Profit: ${estimated_profit:.4f} (Scan was ${opportunity.net_profit_usd:.4f})"
            )
            ...
            if estimated_profit <= 0.05:  # Minimal hurdle
                Logger.warning(
                    f"[EXEC] ❌ Quote slippage ate profit: ${estimated_profit:.4f}"
                )
                return None
            ...
            buy_tx = VersionedTransaction.from_bytes(
                base64.b64decode(tx1_data["swapTransaction"])
            )
            recent_blockhash = buy_tx.message.recent_blockhash
            ...
            Logger.info(
                f"🚀 [JITO] Triangular Bundle (V128.1 Simulation on {getattr(rpc, 'name', 'Jito')})..."
            )
            bundle_id = await self.jito.submit_bundle(
                signed_txs, simulate=True, rpc=rpc
            )

            if bundle_id:
                Logger.info(
                    f"🚀 [JITO] Triangular Bundle Submitted: {bundle_id[:8]}..."
                )
