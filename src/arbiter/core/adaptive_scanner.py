...
class AdaptiveScanner:
    ...
    def sync_with_db(self) -> None:
        ...
        try:
            ...
            self.promoted_pairs = set(
                db_manager.get_high_variance_pairs(hours=1, min_range=0.4)
            )

            # Demote high-LIQ-failure pairs (skip them more)
            liq_rates = db_manager.get_liq_failure_rate(hours=2)
            self.demoted_pairs = {
                pair for pair, rate in liq_rates.items() if rate > 0.7
            }
    ...
    def filter_pairs(self, all_pairs: List[tuple], now: float = None) -> List[tuple]:
        ...
        def get_priority(pair_tuple: tuple) -> float:
            ...
            if now < self.warming_until.get(pair_name, 0):
                ...
                last_scan = (
                    self.pair_metrics.get(pair_name).last_scan_time
                    if pair_name in self.pair_metrics
                    else 0
                )
    ...
    def update_pair(self, opp: SpreadOpportunity) -> None:
        ...
        metrics.spread_variance = (
            alpha * spread_delta + (1 - alpha) * metrics.spread_variance
        )
        ...
        if (
            (is_promising and opp.spread_pct >= self.SPREAD_THRESHOLD)
            or (metrics.spread_variance >= self.WARM_VOLATILITY_THRESHOLD)
            or (is_actionable)
        ):
        ...
        skip_seconds = self._calculate_skip_interval(
            opp.pair, opp.spread_pct, metrics.spread_variance
        )
        metrics.skip_until = now + skip_seconds

    def _calculate_skip_interval(
        self, pair: str, spread_pct: float, variance: float
    ) -> float:
        """
    ...
    def get_priority_pairs(self, all_pairs: List) -> List:
        ...
        now = time.time()
        return sorted(
            all_pairs, key=lambda p: self.warming_until.get(p[0], 0), reverse=True
        )
