...
class FundingRateArbitrage:
    ...
    async def check_opportunity(
        ...
        try:
            ...
            if abs(funding.rate_8h) < self.min_rate_pct:
                Logger.debug(f"[FUNDING] {market} rate {funding.rate_8h:.4f}% below min {self.min_rate_pct}%")
                return None
    ...
    async def enter_position(
        ...
        if market in self.positions and self.positions[market].state == PositionState.ACTIVE:
        ...
        try:
            Logger.info(f"[FUNDING] Entering {market} position: ${opportunity.position_size:.2f}")
            ...
            Logger.info(f"[FUNDING] Would buy {size_base:.4f} {base} spot @ ${mark_price:.2f}")
            ...
            if self.drift.is_connected:
                ...
                position.fees_paid += opportunity.position_size * 0.001  # 0.1% taker fee
            else:
                # Paper mode
                Logger.info(f"[FUNDING] Paper mode - simulated SHORT")
                position.perp_size = size_base
    ...
    async def exit_position(self, market: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.drift and self.drift.is_connected:
                ...
                Logger.info(f"[FUNDING] Paper mode - simulated close")
