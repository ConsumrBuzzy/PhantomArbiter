...
class SpatialArbitrage:
    ...
    def scan_and_execute(
        ...
        if profitable:
            Logger.info(f"[SPATIAL] Found {len(profitable)} opportunities above {self.min_spread}%")
    ...
    async def execute(self, opportunity: SpreadOpportunity) -> dict:
        ...
        try:
            Logger.info(f"[SPATIAL] Executing: {opportunity.pair} | Spread: +{opportunity.spread_pct:.2f}%")
