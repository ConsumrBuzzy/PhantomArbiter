...
class Landlord:
    ...
    async def get_funding_snapshot(self, symbol: str = "SOL-PERP") -> Optional[FundingSnapshot]:
    ...
    async def should_hedge(self, inventory_value_usd: float) -> tuple[bool, str]:
        ...
        if funding.rate_hourly < self.min_funding_rate:
            return False, f"Low funding ({funding.rate_hourly:.3f}%/h < {self.min_funding_rate:.3f}%)"
    ...
    async def open_hedge(self, inventory_value_usd: float) -> Dict[str, Any]:
        ...
        Logger.info(f"[LANDLORD] 📉 Opening SHORT hedge: {size_base:.4f} SOL @ ${funding.mark_price:.2f}")
        ...
        try:
            ...
            Logger.info(f"[LANDLORD] ✅ Hedge opened: {order_result.get('signature', '')[:16]}...")
    ...
    async def close_hedge(self, reason: str = "Manual") -> Dict[str, Any]:
        ...
        try:
            ...
            Logger.info(f"[LANDLORD] ✅ Hedge closed | Funding earned: ${final_funding:.4f}")
    ...
    async def tick(self, inventory_value_usd: float, arb_opportunity: bool = False) -> Dict[str, Any]:
        """
        ...
        if self.state == HedgeState.HEDGED and self.position:
            ...
            if funding:
                ...
                self.position.funding_collected = abs(self.position.size) * funding.rate_hourly / 100 * hours_open
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "position": {
                ...
                "funding_collected": self.position.funding_collected if self.position else 0,
...
if __name__ == "__main__":
    async def test_landlord():
        ...
        if funding:
            ...
            print(f"   Direction: {'Longs pay Shorts ✅' if funding.is_positive else 'Shorts pay Longs'}")
