...
class DashboardFormatter:
    ...
    @staticmethod
    def format_terminal_header(balance: float, gas: float, daily_profit: float, pod_names: List[str] = None, trades: int = 0, volume: float = 0, turnover: float = 0) -> str:
    ...
    @staticmethod
    def format_terminal_row(opp: SpreadOpportunity, status_override: Optional[str] = None) -> str:
        """Format a single row for the terminal table."""
        status = status_override or getattr(opp, 'verification_status', None) or "🔍 SCAN"
        
        # Shorten status for table
        if "LIVE" in status or "READY" in status: status = "✅ READY"
        elif "LIQ" in status: status = "❌ LIQ"
        elif "SLIP" in status: status = "❌ SLIP"
        elif "SCALED" in status: status = "⚠️ SCALED"
    ...
    @staticmethod
    def format_telegram_dashboard(spreads: List[SpreadOpportunity], 
                                  balance: float, 
                                  gas: float, 
                                  daily_profit: float,
                                  pod_names: List[str] = None) -> str:
        ...
        for opp in spreads:
            ...
            if "LIVE" in status_raw or "READY" in status_raw: status_icon = "✅"
            elif "SCALED" in status_raw: status_icon = "⚠️"
            elif "LIQ" in status_raw: status_icon = "💧"
            elif "SLIP" in status_raw: status_icon = "📉"
