...
class MultiverseFragment(BaseFragment):
    ...
    def render(self, state: Any) -> RenderableType:
        ...
        layout.split_column(
            Layout(table),
            Layout(Panel(summary, box=box.SIMPLE, style="dim"))
        )
        
        return Panel(layout, title="[bold cyan]🌌 Multiverse Scanner[/bold cyan]", border_style="cyan")
...
class GraphStatsFragment(BaseFragment):
    ...
    def render(self, state: Any) -> RenderableType:
        ...
        return Panel(grid, title="[bold yellow]📊 Graph Topology[/bold yellow]", border_style="yellow")
...
class JitoBundleFragment(BaseFragment):
    ...
    def render(self, state: Any) -> RenderableType:
        ...
        if not exec_pod:
            return Panel("Waiting for ExecutionPod...", title="[bold red]⚔️ Jito Allocator[/bold red]", border_style="red")
        ...
        return Panel(content, title="[bold red]⚔️ Jito Bundle Preview[/bold red]", border_style="red")
