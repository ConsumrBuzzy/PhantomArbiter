...
class PulsedDashboard:
    ...
    def __init__(self):
        ...
        self.layout["main"].split_row(
            Layout(name="left_container", ratio=3), Layout(name="right", ratio=2)
        )
    ...
    def generate_opp_table(self, spreads: Any, verified_opps: Any = None):
        ...
        return Panel(
            table, title="[bold]Live Market Observer[/bold]", border_style="blue"
        )

    def generate_scalper_panel(
        self, signals: List[Any], market_pulse: Dict[str, Any] = None
    ):
        """Legacy helper - TODO: Migrate to Fragment."""
        if getattr(Settings, "HOP_ENGINE_ENABLED", False):
        ...
        return Panel(
            table,
            title="[magenta]Scalper & Price Watch[/magenta]",
    ...
    def generate_multiverse_panel(self, hop_data: Any = None):
        ...
        return Panel(
            table, title="[cyan]🌌 Multiverse Hop Scanner[/cyan]", border_style="cyan"
        )
    ...
    def generate_signal_panel(self, signals: List[Any]):
        ...
        for sig in list(signals)[:10]:
            ...
            table.add_row(
                ts,
                f"[{color}]{s_type}[/{color}]",
                getattr(sig, "source", "sys"),
                summary,
            )

        return Panel(
            table, title="Signal Intelligence (Global Feed)", border_style="magenta"
        )
    ...
    def generate_graph_stats_panel(self, stats_data: Any = None):
        ...
        return Panel(
            grid, title="[yellow]📊 Graph Engine Stats[/yellow]", border_style="yellow"
        )
    ...
    def update_from_state(self, app_state):
        ...
        if scalper_panel:
            ...
            self.dashboard.layout["scalper"].update(
                self.dashboard.generate_scalper_panel(
                    app_state.scalp_signals, getattr(app_state, "market_pulse", None)
                )
            )
        ...
        self.dashboard.layout["footer"].update(Panel(f"[{now}] {log_msg}", style="dim"))
