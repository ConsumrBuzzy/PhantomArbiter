...
class DataBroker:
    ...
    def __init__(self):
        ...
        # Watchlist Monitor
        self.watchlist_file = os.path.abspath(os.path.join(os.path.dirname(__file__), "data/watchlist.json"))
    ...
    def _get_unified_status(self) -> str:
        ...
        try:
             ...
             if self.market_aggregator:
                 status = loop.run_until_complete(self.market_aggregator.get_unified_status())
             else:
    ...
    def _handle_bitquery_update(self, data: dict):
        ...
        try:
            ...
            if not symbol or not mint: return
    ...
    def _update_market_regime(self):
        ...
        if not self.market_aggregator: return
        ...
        try:
            ...
            try:
                ...
                telemetry = loop.run_until_complete(self.market_aggregator.get_dex_telemetry("SOL"))
            finally:
                loop.close()
    ...
    def _load_watchlist_config(self):
        ...
        try:
            ...
            # Rebuild map
            self.watched_mints = {}
            for symbol, mint in a.items(): self.watched_mints[mint] = symbol
            for symbol, mint in s.items(): self.watched_mints[mint] = symbol
            for symbol, mint in v.items(): self.watched_mints[mint] = symbol
            for symbol, mint in w.items(): self.watched_mints[mint] = symbol
            
            print(f"   📋 Config Loaded: {len(self.watched_mints)} tokens (Active={len(a)}, Scout={len(s)})")
                    ...
                    if hasattr(engine, 'get_held_mints'):
                        ...
                        for sym, mint in held_mints.items():
                            if mint not in self.watched_mints:
                                ...
                                print(f"   🛡️ Keeping {sym} in Data Feed (Held in {name})")
        except Exception as e:
    ...
    def _run_tick(self):
        ...
        try:
            ...
            if prices:
                ...
                self.reporter.send_market_snapshot(batch_prices, wss_stats, priority=priority)
                
                # V48.0: Universal Watcher - Fetch rich market data every 10 batches
                if self.batch_count % 10 == 0:
                    try:
                        symbol_map = {v: k for k, v in self.watched_mints.items()}
                        rich_data = self.universal_watcher.fetch_market_data_batch(all_mints, symbol_map)
                        ...
                        if rich_data:
                            ...
                            for mint, mkt_data in rich_data.items():
                                if mkt_data.price_change_1h and mkt_data.price_change_1h > 300:
            ...
            if self.batch_count % 5 == 0:
                ...
                if is_paper_diagnostic and (all_signals or winning_signals):
                    ...
                    for sig in winning_signals:
                        print(f"   🎯 {sig.get('symbol')} {sig.get('action')} (engine: {sig.get('engine')})")
                
                for signal in winning_signals:
                    engine = self.merchant_engines.get(signal['engine'])
                    if engine:
                        if is_paper_diagnostic:
                            print(f"   🚀 Executing {signal.get('symbol')} {signal.get('action')}...")
    ...
    def _scan_and_cache_wallet(self):
        ...
        try:
            ...
            if held_assets:
                print(f"   💼 Wallet: ${usdc_balance:.2f} USDC | Bags: {list(held_assets.keys())}")
        except Exception as e:
        ...
        try:
                    ...
                    if success:
                        ...
                        if hasattr(self, 'merchant_engines'):
                            for engine in self.merchant_engines.values():
                                if hasattr(engine, 'reload_ml_model'): engine.reload_ml_model()
            ...
            if cmd_type == CMD_PERFORMANCE:
                ...
                try:
                    ...
                    send_telegram(f"❌ Report Error: {e}", source="PERF", priority="HIGH")
                return
    ...
    def _backfill_history(self):
        ...
        if hasattr(self, 'held_symbols') and self.held_symbols:
            ...
            symbols = held + others
            print(f"   📊 Backfill Order: {len(held)} Priority + {len(others)} Standard")
        ...
        def check_backfill(sym):
            ...
            if len(hist) < 50: return True, f"insufficient ({len(hist)} points)"
            if hist:
                age = time.time() - max(h.get("ts", 0) for h in hist)
                if age > stale_threshold: return True, f"stale ({int(age/60)}m old)"
            return False, ""
        ...
        if background:
            ...
            def run_bg_backfill():
                for i, (symbol, reason, _) in enumerate(background):
                    ...
                    if i > 0: time.sleep(10) # Slow pace for background
                ...
                if prices:
                    ...
                    with lock:
                        ...
                        if symbol not in cache_data["prices"]:
                            cache_data["prices"][symbol] = {"price": 0, "source": "", "timestamp": 0, "history": []}
...
class BrokerPriceCache:
    ...
    def _fire_discovery_trigger(self, mint: str):
        ...
        t = threading.Thread(target=run_trigger, daemon=True, name=f"Trigger-{mint[:4]}")
        t.start()
