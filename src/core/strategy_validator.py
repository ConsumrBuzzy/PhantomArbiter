...
class StrategyValidator:
    ...
    def validate_buy(self, symbol: str, mint: str = None, timeframe_hours: int = 24) -> tuple:
        """
    ...
    def _run_simulation(self, symbol: str, price_history: list) -> list:
        ...
        history_subset = price_history[-2000:] if len(price_history) > 2000 else price_history
...
if __name__ == "__main__":
    ...
    for symbol in assets:
        ...
        print(f"{status} {symbol}: Win Rate {stats['win_rate']:.1f}% ({stats['count']} trades) Avg PnL: {stats.get('avg_pnl', 0):.2f}%")
