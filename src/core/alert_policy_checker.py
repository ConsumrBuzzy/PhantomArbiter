        ...
        try:
            ...
            try:
                metrics = loop.run_until_complete(
                    self.market_aggregator.get_raw_metrics()
                )
            finally:
                loop.close()
            ...
            if metrics.get("atr_pct", 0) > atr_threshold:
                ...
                if self._can_send_alert(alert_key, cooldown):
                    alerts.append(
                        f"🚨 **DEX ALERT:** Volatility HIGH! ATR: {metrics['atr_pct'] * 100:.2f}%"
                    )
            ...
            if metrics.get("adx", 0) > adx_threshold:
                ...
                if self._can_send_alert(alert_key, cooldown):
                    alerts.append(
                        f"📈 **TREND ALERT:** Strong trend detected! ADX: {metrics['adx']:.1f}"
                    )
                    self._mark_alert_sent(alert_key)
            ...
            if (
                metrics.get("dydx_margin_ratio", 1.0) < margin_threshold
                and metrics.get("dydx_equity", 0) > 0
            ):
                alert_key = "dydx_margin"
                if self._can_send_alert(alert_key, cooldown):
                    alerts.append(
                        f"⚠️ **dYdX RISK:** Margin low at {metrics['dydx_margin_ratio'] * 100:.1f}%!"
                    )
                    self._mark_alert_sent(alert_key)
