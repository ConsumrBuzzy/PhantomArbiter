...
class JupiterProvider(PriceProvider):
    ...
    def fetch_prices(self, mints: list) -> dict:
        ...
        for chunk in chunks:
            ids = ",".join(chunk)
            url = (
                f"https://price.jup.ag/v6/price?ids={ids}&vsToken={Settings.USDC_MINT}"
            )
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        if attempt == 0:
                            ...
                            raise Exception("Jupiter 429 Rate Limit")
                    else:
                        if attempt == 1:
                            raise Exception(f"Jupiter Status {resp.status_code}")
