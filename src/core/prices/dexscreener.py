...
class DexScreenerProvider(PriceProvider):
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    if pairs:
                        ...
                        Logger.warning(
                            f"      ⚠️ DexScreener returned no pairs for {len(chunk)} mints"
                        )

                elif resp.status_code == 429:
                    Logger.warning("      ⚠️ DexScreener Rate Limit (429)")
    ...
    def fetch_market_data(
        ...
        try:
            ...
            if resp.status_code != 200:
                Logger.warning(
                    f"⚠️ DexScreener returned {resp.status_code} for {symbol}"
                )
                return None
    ...
    def fetch_market_data_batch(
        self, mints: List[str], symbol_map: Dict[str, str] = None
    ) -> Dict[str, MarketData]:
        """
            ...
            try:
                ...
                for original_mint, token_pairs in mint_pairs.items():
                    ...
                    if primary:
                        ...
                        market_data = self._parse_pair_to_market_data(
                            original_mint, symbol, primary
                        )
                        if market_data:
    ...
    def _parse_pair_to_market_data(
        self, mint: str, symbol: str, pair: dict
    ) -> Optional[MarketData]:
...
if __name__ == "__main__":
    ...
    for symbol, mint in TEST_TOKENS.items():
        ...
        if market_data:
            ...
            print(
                f"   🔄 Buys/Sells 24h: {market_data.txns_buys_24h}/{market_data.txns_sells_24h}"
            )
    ...
    for mint, data in batch_results.items():
        print(
            f"   • {data.symbol}: ${data.price_usd:.6f} via {data.dex_id.upper()} (Liq: ${data.liquidity_usd:,.0f})"
        )
