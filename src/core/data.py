...
class CircuitBreaker:
    ...
    def record_failure(self):
        ...
        if self.failures >= self.failure_threshold and not self.is_open:
            self.is_open = True
            Logger.warning(
                f"      ⚡ CIRCUIT BREAKER TRIPPED: Switching to DexScreener for {self.cooldown_seconds}s"
            )
    ...
    for i, chunk in enumerate(chunks):
        ...
        if not chunk_success:
            if use_jupiter:  # Only warn if we actually tried Jupiter and it failed
                Logger.warning(
                    f"      ⚠️ Jupiter Batch Failed ({len(chunk)}/30). Trying DexScreener..."
                )
            ...
            try:
                ...
                if ds_resp.status_code == 200:
                    ...
                    for pair in ds_data.get("pairs", []):
                        ...
                        if (
                            base_mint
                            and base_mint in chunk
                            and base_mint not in all_results
                        ):
...
class DataFeed:
    ...
    def __init__(self, mint=None, symbol=None, is_critical=False, lazy_init=False):
        ...
        if is_broker_alive():
            ...
            if len(history) >= 50:
                ...
                Logger.info(
                    f"   📡 {self.symbol}: Loaded {len(history)} points from broker cache"
                )
            ...
            if ds_resp.status_code == 200:
                ...
                if pairs:
                    ...
                    self.liquidity_usd = float(
                        pair.get("liquidity", {}).get("usd", 0.0)
                    )
    ...
    def resolve_coingecko_id(self, mint):
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if cg_id:
                    Logger.info(
                        f"   🔍 Resolved CoinGecko ID for {self.symbol}: {cg_id}"
                    )
    ...
    def _fetch_coingecko_history(self, mint):
        ...
        if time.time() < CG_429_COOLDOWN_UNTIL:
            ...
            Logger.debug(
                f"      ⏳ CoinGecko in cooldown ({remaining}s remaining), skipping"
            )
            return [], []
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if resp.status_code == 200:
                    ...
                    Logger.warning("      ⚠️ CG Rate Limit (429) - Cooldown 5min")
                    return [], []
                else:
            ...
            if os.path.exists(self.history_file):
                ...
                if time.time() - last_ts < 3600:  # 1 hour
                    ...
                    if len(prices) >= 15:
                        ...
                        Logger.info(
                            f"   📂 Loaded {len(self.raw_prices)} prices from disk. RSI: {self.current_rsi}"
                        )
                        return True
                    else:
                        Logger.warning(
                            f"   ⚠️ Cached history too short ({len(prices)} prices)"
                        )
                else:
                    Logger.warning(
                        f"   ⚠️ Cached history stale (>{(time.time() - last_ts) / 60:.0f}m old)"
                    )
        except Exception as e:
    ...
    def backfill_history(self):
        ...
        try:
            if self.is_critical:
                Logger.info("   ⏳ Fetching history from CoinGecko (CRITICAL)...")
            else:
                Logger.info("   ⏳ Fetching history from CoinGecko (Fast-Fail)...")
            ...
            if prices:
                ...
                self.last_source = "CG"
                Logger.success(
                    f"   ✅ Backfilled {len(prices)} points. Initial RSI: {self.current_rsi}"
                )
                ...
                try:
                    ...
                    with lock:
                        ...
                        # Build history array
                        history = [
                            {"price": p, "ts": time.time()} for p in prices[-200:]
                        ]
                ...
                Logger.warning("   ⚠️ CoinGecko returned no data")
        except Exception as e:
    ...
    def calculate_rsi(self, period=14):
        if len(self.raw_prices) < period + 1:
            return 50.0
