...
class MarketAggregator:
    ...
    async def get_unified_status(self) -> str:
        ...
        if dex:
            vol_emoji = "🔴" if dex.volatility == "HIGH" else "🟡" if dex.volatility == "MEDIUM" else "🟢"
            trend_emoji = "📈" if "TREND" in dex.trend else "↔️" if dex.trend == "RANGING" else "⏸️"
            
            sections.append("")
            sections.append("🔗 **DEX Market (Solana)**")
            sections.append(f"Price: ${dex.price:,.4f}")
            sections.append(f"{vol_emoji} Volatility: **{dex.volatility}** (ATR: {dex.atr_pct*100:.2f}%)")
            sections.append(f"{trend_emoji} Trend: **{dex.trend}** (ADX: {dex.adx:.1f})")
        ...
        if dydx:
            health_emoji = "🟢" if dydx["margin_ratio"] > 0.5 else "🟡" if dydx["margin_ratio"] > 0.2 else "🔴"
    ...
    async def get_jlp_status(self, jlp_state: dict) -> dict:
        ...
        if not jlp_state or jlp_state.get("quantity", 0) == 0:
            result["message"] = "JLP monitoring inactive. Buy JLP and run /set_jlp [price] [quantity]"
            return result
        ...
        result["profit_pct"] = (result["profit"] / initial_value * 100) if initial_value > 0 else 0
        result["ready"] = True
