...
class ActiveCoinAuditor:
    ...
    def run_audit(self):
        ...
        if active_coins:
            ...
            for symbol, mint in active_coins:
                ...
                if win_rate >= self.min_win_rate or trade_count == 0:
                    ...
                    Logger.info(
                        f"      ✅ {symbol}: WR={win_rate:.1f}% ({trade_count} trades)"
                    )
                else:
                    demoted.append(symbol)
                    Logger.warning(
                        f"      ⚠️ {symbol}: WR={win_rate:.1f}% < {self.min_win_rate}% - DEMOTING"
                    )
        ...
        if scout_coins:
            Logger.info(
                f"   🔭 Checking {len(scout_coins)} Scout coins for promotion..."
            )
            ...
            for symbol, mint in scout_coins:
                ...
                if (
                    win_rate >= self.promotion_min_win_rate
                    and trade_count >= self.promotion_min_trades
                ):
                    promoted.append(symbol)
                    Logger.success(
                        f"      ⬆️ {symbol}: WR={win_rate:.1f}%, {trade_count} trades - PROMOTING!"
                    )
                    continue
                ...
                try:
                    ...
                    if liq_usd > 50000:
                        promoted.append(symbol)
                        Logger.success(
                            f"      🛡️ {symbol}: Tier 1 Failed ({reason}) but Liquidity ${liq_usd / 1000:.0f}k > $50k - PROMOTING (Tier 2)"
                        )
                    else:
                        Logger.warning(
                            f"      🔭 {symbol}: Tier 1 Failed & Low Liq (${liq_usd / 1000:.0f}k < $50k) - REMAINING IN SCOUT"
                        )
                except Exception as e:
        ...
        # Summary
        Logger.section(
            f"📋 AUDIT COMPLETE: Active: {len(passed)} OK, {len(demoted)} Demoted | Scout: {len(promoted)} Promoted"
        )
