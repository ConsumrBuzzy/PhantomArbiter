...
class TokenValidator:
    ...
    def __init__(self):
        self.cache_file = os.path.join(os.path.dirname(__file__), "validation_cache.json")
    ...
    def validate(self, mint: str, symbol: str = "UNKNOWN") -> ValidationResult:
        ...
        is_paper_diagnostic = (
            not Settings.ENABLE_TRADING and 
            getattr(Settings, 'PAPER_AGGRESSIVE_MODE', False)
        )
        ...
        is_safe = mint_ok and freeze_ok and honeypot_ok and liquidity_ok and top_holders_ok
        ...
        if not top_holders_ok:
            reasons.append(f"Top 10 hold {top10_pct*100:.1f}% (> {Settings.MAX_TOP10_HOLDER_PCT*100:.0f}%)")
            ...
            if "result" in data and data["result"] and "value" in data["result"]:
                ...
                if value and "data" in value:
                    ...
                    if mint_authority is None:
                        ...
                        if self.VERBOSE: print(f"      ⚠️ Mint Authority: {mint_authority[:8]}...")
                        return False  # Active = Unsafe
            
            # If we can't determine, assume safe (established tokens)
            if self.VERBOSE: print(f"      ⚠️ Could not verify mint authority")
            return True
            
        except Exception as e:
            if self.VERBOSE: print(f"      ❌ Mint authority check failed: {e}")
            ...
            if "result" in data and data["result"] and "value" in data["result"]:
                ...
                if value and "data" in value:
                    ...
                    if freeze_authority is None:
                        ...
                        if self.VERBOSE: print(f"      ⚠️ Freeze Authority: {freeze_authority[:8]}...")
                        return False  # Active = Unsafe
            
            if self.VERBOSE: print(f"      ⚠️ Could not verify freeze authority")
            return True
            
        except Exception as e:
            if self.VERBOSE: print(f"      ❌ Freeze authority check failed: {e}")
            return True
    ...
    def check_honeypot(self, mint: str) -> bool:
                ...
                if resp.status_code == 200:
                    ...
                    if "outAmount" in data:
                        ...
                        if out_amount > 0:
                            ...
                            if self.VERBOSE: print(f"      ⚠️ Honeypot: Sell returns $0")
                            return False
                    
                    # Check for error response
                    if "error" in data:
                        if self.VERBOSE: print(f"      ⚠️ Honeypot: {data['error']}")
                        return False
                
                # No route found could mean low liquidity, not necessarily honeypot
                if resp.status_code == 400:
                    if self.VERBOSE: print(f"      ⚠️ No sell route found (low liquidity?)")
        ...
        if self.VERBOSE: print(f"      ⚠️ Honeypot check skipped (API unavailable)")
        return True
    ...
    def check_liquidity(self, mint: str) -> Tuple[bool, float]:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if not pairs:
                    if self.VERBOSE: print(f"      ⚠️ No trading pairs found")
                    return False, 0.0
                ...
                if not Settings.ENABLE_TRADING and hasattr(Settings, 'PAPER_MIN_LIQUIDITY'):
                ...
                if not is_sufficient:
                    if self.VERBOSE: print(f"      ⚠️ Low liquidity: ${max_liquidity:,.0f} < ${threshold:,.0f}")
                
                return is_sufficient, max_liquidity
            
            if self.VERBOSE: print(f"      ⚠️ DexScreener API error: {resp.status_code}")
            return True, 0.0  # Fail open
            
        except Exception as e:
            if self.VERBOSE: print(f"      ❌ Liquidity check failed: {e}")
            return True, 0.0
    ...
    def check_top_holders(self, mint: str) -> Tuple[bool, float]:
        ...
        try:
            ...
            if total_supply <= 0:
                if self.VERBOSE: print("      ⚠️ Could not fetch total supply")
                return True, 0.0 # Fail open
            ...
            if not is_safe:
                if self.VERBOSE: print(f"      ⚠️ High Concentration: Top 10 hold {concentration*100:.1f}%")
            
            return is_safe, concentration
            
        except requests.exceptions.Timeout:
            if self.VERBOSE: print("      ⚠️ RPC Timeout - skipping holder check")
            return True, 0.0  # Fail open
        except requests.exceptions.ConnectionError:
            if self.VERBOSE: print("      ⚠️ RPC Connection Error - skipping holder check")
            return True, 0.0  # Fail open
        except Exception as e:
            if self.VERBOSE: print(f"      ⚠️ Holder check failed: {str(e)[:50]}")
            return True, 0.0 # Fail open
        ...
        try:
            ...
            if "result" in data and data["result"] and "value" in data["result"]:
                ...
                if value:
                    ...
                    if is_2022:
                        if self.VERBOSE: print(f"      ⚠️ Token-2022 DETECTED: {mint[:8]}...")
            ...
            if self.VERBOSE: print(f"      ⚠️ Token-2022 check failed: {e}")
