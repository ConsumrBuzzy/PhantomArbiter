...
class MerchantEnsemble(BaseStrategy):
    ...
    def __init__(self, portfolio):
        ...
        Logger.info(
            "🧠 [ENSEMBLE] Merchant Mind initialized (V67.5 Weighted Consensus)"
        )
    ...
    def _calculate_weighted_consensus(
        self, ml_score: float, symbol: str
        ...
        try:
            ...
            if pnl_24h < 0:
                ...
                weight_whale = 0.30
                Logger.debug(
                    "[ENSEMBLE] Dynamic Weights: ML underperforming, boosting Scout"
                )
        except Exception:
        ...
        relevant_signals = [
            s
            for s in self.pending_agent_signals
            if s.symbol == symbol or s.symbol == "UNKNOWN"
        ]
        ...
        self.pending_agent_signals = [
            s for s in self.pending_agent_signals if s not in relevant_signals
        ]
    ...
    def analyze_tick(
        self, watcher, price: float, agent_signals: List[AgentSignal] = None
    ) -> tuple[str, str, float]:
        """
        ...
        for name, strategy in self.sub_strategies.items():
            try:
                ...
                if action in ["BUY", "SELL"]:
                    ...
                    if name == "SCALPER":
                        confidence = 0.6
                    if name == "KELTNER" and "ENTRY" in reason:
                        confidence = 0.8
                    if name == "VWAP" and "ENTRY" in reason:
                        confidence = 0.7
        ...
        if buy_signals:
            ...
            final_score, agent_reason, should_boost = (
                self._calculate_weighted_consensus(avg_ml_conf, watcher.symbol)
            )
            ...
            if len(buy_signals) >= 2:
                ...
                return (
                    "BUY",
                    f"🧠 CONSENSUS ({sources}) {agent_reason} | Score:{final_score:.2f}",
                    final_size,
                )
            ...
            return (
                s["action"],
                f"{s['source']}: {s['reason']} {agent_reason} | Score:{final_score:.2f}",
                final_size,
            )
        ...
        if self.pending_agent_signals:
            for sig in self.pending_agent_signals:
                source = (
                    sig.metadata.get("source", "UNKNOWN") if sig.metadata else "UNKNOWN"
                )
                ...
                if source in ["SNIPER", "WHALE_WATCHER"] and sig.action == "BUY":
                    ...
                    size = (
                        sig.metadata.get("snipe_size", 10.0) if sig.metadata else 10.0
                    )
                    ...
                    Logger.info(
                        f"🎯 [ENSEMBLE] Agent-Only Trade: {source} {sig.symbol} @ {sig.confidence:.0%}"
                    )

                    # ═══ V12.6: Push to TUI AppState ═══
                    try:
                        from src.shared.state.app_state import (
                            state as app_state,
                            ScalpSignal,
                        )

                        app_state.add_signal(
                            ScalpSignal(
                                token=sig.symbol,
