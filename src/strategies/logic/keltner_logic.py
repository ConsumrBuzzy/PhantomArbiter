...
class KeltnerLogic(BaseStrategy):
    ...
    def update_market_mode(self):
        ...
        Logger.info(
            f"🌊 Keltner Mode: EMA 20 / ATR 10 (Win Rate: {self.win_rate * 100:.1f}%)"
        )
    ...
    def analyze_tick(self, watcher: Watcher, price: float) -> tuple:
        ...
        if watcher.in_position:
            ...
            if price > ema:
                return (
                    "SELL",
                    self.get_net_pnl_reason(
                        watcher, price, "🌊 KELTNER MEAN REVERSION"
                    ),
                    0.0,
                )
        ...
        if size_usd < 5.0:
            return "HOLD", "", 0.0
        if watcher.get_price_count() < 20:
            return "HOLD", "", 0.0  # V70.5: Reduced from 30
