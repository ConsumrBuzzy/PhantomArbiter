...
class TacticalStrategy:
    ...
    def __init__(
        ...
        if strategy_class:
            ...
            priority_queue.add(
                3,
                "LOG",
                {"level": "INFO", "message": f"🧠 Strategy: {strategy_class.__name__}"},
            )
        else:
        ...
        self.watcher_mgr = WatcherManager(
            validator=self.validator, engine_name=engine_name
        )
    ...
    async def initialize(self):
        ...
        if not Settings.ENABLE_TRADING:
            ...
            try:
                ...
                initial_capital_data = await asyncio.to_thread(
                    self.wallet.get_current_live_usd_balance
                )
                ...
                Logger.info(
                    f"✅ [{self.engine_name}] Capital Initialized: ${initial_capital:,.2f}"
                )
        ...
        if self.execution_mode == "DYDX" or dydx_enabled:
            try:
                ...
                self.dydx_adapter = DydxAdapter(
                    getattr(Settings, "DYDX_NETWORK", "testnet")
                )
                # Only pass mnemonic for full trading; otherwise HTTP-only mode
                mnemonic = (
                    getattr(Settings, "DYDX_MNEMONIC", "")
                    if self.execution_mode == "DYDX"
                    else ""
                )
                ...
                priority_queue.add(
                    ...
                    {
                        "level": "INFO",
                        "message": f"🌐 [{self.engine_name}] dYdX: {self.dydx_adapter}",
                    },
                )
            except Exception as e:
                priority_queue.add(
                    ...
                    {
                        "level": "WARN",
                        "message": f"⚠️ [{self.engine_name}] dYdX init failed: {e}",
                    },
                )
        ...
        try:
            ...
            model_path = os.path.join(
                os.path.dirname(__file__), "..", "..", "models", "ml_filter.pkl"
            )
            if os.path.exists(model_path):
                self.ml_model = joblib.load(model_path)
                self._ml_model_mtime = os.path.getmtime(
                    model_path
                )  # V47.0: Track for hot-reload
                priority_queue.add(
                    3,
                    "LOG",
                    {"level": "INFO", "message": "🧠 ML Filter Loaded (V41.0 XGBoost)"},
                )
            else:
                ...
                priority_queue.add(
                    ...
                    {
                        "level": "INFO",
                        "message": "🧠 ML Filter: No model found (Run train_model.py)",
                    },
                )
        except Exception as e:
            priority_queue.add(
                3, "LOG", {"level": "WARN", "message": f"⚠️ ML Filter Load Error: {e}"}
            )
        ...
        if Settings.ENABLE_TRADING:
            self.execution_backend = LiveBackend(
                swapper=self.swapper, engine_name=self.engine_name
            )
        else:
            self.execution_backend = PaperBackend(
                capital_manager=self.capital_mgr, engine_name=self.engine_name
            )
        ...
        try:
            try:
                ...
                Logger.info(
                    "⚠️ [CORE] phantom_core not found, using Python ScorerConfig fallback"
                )
            ...
            self.slippage_calibrator = SlippageCalibrator(
                scorer_config=scorer_config, shadow_manager=self.shadow_manager
            )
        ...
        self.config_sync = ConfigSyncService(
            engine_name=self.engine_name, portfolio=self.portfolio
        )
        ...
        def handle_scout_signal(sig: Signal):
            ...
            mint = sig.data.get("mint") or sig.data.get(
                "symbol"
    ...
    def set_live_mode(self, live: bool):
        ...
        Logger.info(
            f"🔄 [{self.engine_name}] Switching to {'LIVE' if live else 'PAPER'} mode..."
        )
        ...
        try:
            ...
            if not hasattr(self, "_ml_model_mtime") or new_mtime > self._ml_model_mtime:
                ...
                priority_queue.add(
                    ...
                    {
                        "level": "SUCCESS",
                        "message": f"[{self.engine_name}] ML Model Hot-Reloaded (V47.0)",
                    },
                )
                return True
        except Exception as e:
            priority_queue.add(
                ...
                {
                    "level": "WARN",
                    "message": f"[{self.engine_name}] ML reload failed: {e}",
                },
            )
        return False
    ...
    def set_trading_mode(self, is_live: bool):
        ...
        priority_queue.add(
            3, "LOG", {"level": "WARNING", "message": f"[{self.engine_name}] {msg}"}
        )
    ...
    def _perform_maintenance_and_heartbeat(self):
        ...
        if now - self.last_heartbeat >= 60:
            ...
            scout_positions = sum(
                1 for w in self.scout_watchers.values() if w.in_position
            )
            ...
            uptime_min = (
                int((now - getattr(self, "start_time", now)) / 60)
            ...
            if not Settings.ENABLE_TRADING and self.paper_wallet.initialized:
                ...
                for s, w in self.scout_watchers.items():
                    price_map[s] = w.get_price()
                ...
                pct = (
                    (diff / self.paper_wallet.initial_capital) * 100
                    if self.paper_wallet.initial_capital > 0
                    else 0
                )
                ...
                if self.paper_wallet.assets:
                    ...
                    for sym, asset in list(self.paper_wallet.assets.items())[:5]:
                        ...
                        pnl_pct = (
                            ((asset_price - asset.avg_price) / asset.avg_price * 100)
                            if asset.avg_price > 0
                            else 0.0
                        )
                        emoji_bag = (
                            "🟢" if pnl_pct > 0 else "🔴" if pnl_pct < 0 else "⚪"
                        )

                        qty_str = (
                            f"{asset.balance:.0f}"
                            if asset.balance > 100
                            else f"{asset.balance:.3f}"
                        )
                        paper_bags.append(
                            f"{emoji_bag} {sym}: {qty_str} (${asset_val:.2f}) {pnl_pct:+.1f}%"
                        )
                ...
                priority_queue.add(
                    ...
                    {
                        "level": "INFO",
                        "message": f"[{engine}] 🎰 PAPER: ${paper_val:.2f} ({pct:+.2f}%) | Bags: {bag_count} | Gas: {gas_bal:.3f} SOL | REAL: ${real_val:.2f}",
                    },
                )
            ...
            if (
                hasattr(self, "  ")
                and self.dydx_adapter
                and self.dydx_adapter.is_connected
            ):
                try:
            ...
            if not paper_section:
                priority_queue.add(
                    ...
                    {
                        "level": "INFO",
                        "message": f"[{engine}] 💓 {self.tick_count}t | {active_positions}A/{scout_positions}S | ${usdc_bal:.0f} | {sol_bal:.2f} SOL",
                    },
                )
    ...
    def execute_signal(self, signal: dict):
        ...
        size_usd = (
            signal.get("size_usd") or Settings.POSITION_SIZE_USD
        )  # V133: Default if None
        ...
        if not watcher:
            ...
            Logger.debug("🛑 REJECTED: No watcher in signal")
            return
        if price is None or price <= 0:
            ...
            if price is None or price <= 0:
                ...
                Logger.debug(
                    f"🛑 REJECTED: No valid price for {getattr(watcher, 'symbol', 'unknown')}"
                )
                return
        ...
        if not hasattr(self, "executor") or self.executor is None:
            ...
            Logger.debug("🛑 REJECTED: Executor not initialized")
            return

        # V85.1: Enforce Dynamic Confidence Thresholds
        is_live = Settings.ENABLE_TRADING
        min_conf = (
            getattr(Settings, "LIVE_MIN_CONFIDENCE", 0.85)
            if is_live
            else getattr(Settings, "PAPER_MIN_CONFIDENCE", 0.45)
        )
        ...
        if action == "BUY" and confidence < min_conf:
            ...
            Logger.debug(
                f"🛑 REJECTED: Low Confidence {confidence:.2f} < {min_conf:.2f} (Paper: {'OFF' if is_live else 'ON'})"
            )
            return
        ...
        if action == "BUY":
            self.executor.execute_buy(
                watcher, price, reason, size_usd, self.decision_engine
            )
        elif action == "SELL":
