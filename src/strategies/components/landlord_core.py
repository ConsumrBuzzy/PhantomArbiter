...
class LandlordCore:
    ...
    async def start_landlord(self, size_usd: float) -> Dict[str, Any]:
        ...
        if self.position.state != LandlordState.IDLE:
            result["message"] = (
                f"Cannot start: Already in {self.position.state.value} state"
            )
            return result

        if size_usd < LANDLORD_CONFIG["MIN_COLLATERAL_USD"]:
            result["message"] = (
                f"Size too small. Minimum: ${LANDLORD_CONFIG['MIN_COLLATERAL_USD']}"
            )
            return result
        ...
        try:
            ...
            Logger.info(
                f"[LANDLORD] Position size: {sol_size:.4f} SOL @ ${sol_price:.2f}"
            )
            ...
            short_result = await drift.place_perp_order(
                symbol="SOL-PERP", direction="SHORT", size=sol_size
            )
            ...
            Logger.info("[LANDLORD] Using existing SOL balance as long leg")

            # ═══════════════════════════════════════════════════════════════
            # STEP 5: Calculate hedge metrics
            # ═══════════════════════════════════════════════════════════════
            self.position.hedge_ratio = self.position.short_size / max(
                self.position.spot_size, 0.0001
            )
    ...
    async def close_landlord(self) -> Dict[str, Any]:
        ...
        try:
            ...
            Logger.info(
                f"[LANDLORD] ✅ SHORT closed: {close_result.get('signature', '')[:16]}..."
            )
            ...
            Logger.info("[LANDLORD] ✅ Returned to IDLE")
            return result
    ...
    async def check_rebalance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            Logger.debug(
                f"[LANDLORD] Hedge check: Ratio={new_hedge_ratio:.3f}, Drift={drift_pct * 100:.1f}%"
            )

            # Rebalance if drift exceeds threshold
            if drift_pct > LANDLORD_CONFIG["REBALANCE_THRESHOLD"]:
                Logger.info(
                    f"[LANDLORD] Rebalancing: Drift {drift_pct * 100:.1f}% > {LANDLORD_CONFIG['REBALANCE_THRESHOLD'] * 100}%"
                )
                ...
                Logger.info("[LANDLORD] ✅ Rebalanced to 1.0")
    ...
    def _start_rebalance_task(self):
        ...
        self._rebalance_task = threading.Thread(
            target=self._rebalance_loop, daemon=True, name="LandlordRebalance"
        )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_rebalance": str(self.position.last_rebalance)
            if self.position.last_rebalance
            else None,
        }
