...
class SlippageCalibrator:
    ...
    def __init__(
        ...
        Logger.info(
            f"⚙️ [CALIBRATOR] Initialized (window={window_size}, range={min_bps}-{max_bps} bps)"
        )
    ...
    def force_loosen(self, reason: str = "Manual") -> None:
        ...
        Logger.warn(
            f"⚠️ [CALIBRATOR] EMERGENCY LOOSEN: {old}→{self.max_bps} bps ({reason})"
        )
    ...
    def force_tighten(self, reason: str = "Manual") -> None:
        ...
        Logger.info(
            f"🔒 [CALIBRATOR] FORCE TIGHTEN: {old}→{self.min_bps} bps ({reason})"
        )
