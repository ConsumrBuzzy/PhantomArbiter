...
async def liquidity_cycle_loop():
    ...
    send_telegram("🐋 Orca CLMM Started (SIMULATION MODE)", source="ORCA", priority="HIGH")
    ...
    while not SHUTDOWN_EVENT.is_set():
        try:
            ...
            active_positions = len([p for p in liquidity_manager.positions.values() if p.is_active])
            
            if active_positions == 0:
                available = cap_man.get_available_cash("UNIFIED") if hasattr(cap_man, 'get_available_cash') else 100
                should_deploy, reason = liquidity_manager.should_deploy(current_regime, available)
