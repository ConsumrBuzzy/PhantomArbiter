...
async def discovery_monitor_loop():
    ...
    async def report_silent_launches():
        ...
        while True:
            ...
            if resolved_counts > 0 or still_unknown_counts:
                msg = "🔭 <b>DISCOVERY SUMMARY (5m)</b>\n━━━━━━━━━━━━━━━\n"

                if resolved_lines:
                    msg += (
                        f"<i><b>✅ Resolved ({len(resolved_lines)}):</b></i>\n"
                        + "\n".join(resolved_lines[:15])
                    )  # Limit to 15 to avoid huge msgs
    ...
    async def on_launch(event: LaunchEvent):
        ...
        display_name = (
            f"{event.name} ({event.symbol})" if event.name else event.mint[:8]
        )
        ...
        Logger.info(
            f"[DISCOVERY] 🚀 NEW LAUNCH: {display_name} [{event.platform.value}]"
        )
    ...
    async def on_migration(event: MigrationEvent):
        ...
        Logger.info(
            f"[DISCOVERY] 🔄 MIGRATION: {event.mint[:8]}... → {event.destination_dex}"
        )
    ...
    async def on_opportunity(opp: MigrationOpportunity):
        ...
        if confidence_value >= min_confidence:
            ...
            Logger.success(
                f"[DISCOVERY] 🎯 SNIPE OPP: {opp.mint[:8]}... ({opp.confidence.name})"
            )
            send_telegram(msg, source="DISCOVERY", priority="HIGH")
            ...
            if auto_trade:
                ...
                success, msg = capital_mgr.execute_buy(
                    ...
                    dex_id=opp.destination_dex.upper()
                    or "UNKNOWN",  # Tag source (e.g. RAYDIUM)
                )
    ...
    try:
        ...
        monitor.stop()
        send_telegram(
            "🔍 Multi-Pad Discovery Stopped", source="DISCOVERY", priority="MEDIUM"
        )
