...
class RugCheck:
    ...
    def validate_strict(self, mint: str) -> tuple:
        ...
        details = {
            ...
            "risks": [
                r.get("name", "Unknown") if isinstance(r, dict) else str(r)
                for r in risks
            ]
            if risks
            else [],
        ...
        if lp_locked_pct > 0 and lp_locked_pct < self.min_lp_locked_pct:
            return (
                False,
                f"Low LP Lock ({lp_locked_pct:.1f}% < {self.min_lp_locked_pct}%)",
                details,
            )
...
if __name__ == "__main__":
    ...
    is_safe, reason, details = rc.validate_strict(
        "7GCihgDB8fe6KNjn2MYtkzZcRjQy3t9GHdC8uHYmW2hr"
    )
    print(f"Result: {'✅ SAFE' if is_safe else '❌ UNSAFE'} - {reason}")
    print(
        f"Details: Score={details.get('score')}, LP={details.get('lp_locked_pct', 0):.1f}%"
    )
