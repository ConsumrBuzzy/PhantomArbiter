...
if positions:
    bag_value = sum(
        [p["amount"] * current_prices.get(t, 0) for t, p in positions.items()]
    )
...
col1.metric(
    "❤️ Total HP (Portfolio)",
    f"${total_hp:.2f}",
    delta=f"{((total_hp / start_hp) - 1) * 100:.1f}%",
)
            ...
            for j in range(3):
                if i + j < len(tickers):
                    ...
                    with cols[j]:
                        ...
                        price_hist = (
                            [p["price"] for p in hist_data] if hist_data else []
                        )
                        ...
                        if len(price_hist) > 10:
                            # Verify price logic
                            start_p = (
                                price_hist[-2] if len(price_hist) > 1 else price_hist[0]
                            )
                            end_p = price_hist[-1]
                            ...
                            st.markdown(
                                f"**RSI:** {rsi_val:.1f} | **Status:** :{status_color}[{status_text}]"
                            )
...
with tab2:
    if not positions:
        ...
        if "wallet" in market_data:
            st.caption(
                f"Raw Wallet Data: {market_data['wallet'].get('held_assets', '{}')}"
            )
