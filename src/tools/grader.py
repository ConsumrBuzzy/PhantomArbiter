...
class TokenGrader:
    ...
    def __init__(self):
        self.history_file = os.path.join(
            os.path.dirname(__file__), "../../config/grading_history.json"
        )
    ...
    def grade_token(self, symbol: str, metrics: dict) -> float:
        ...
        liquidity_score = min(
            100, metrics.get("liquidity_ratio", 0) * 100
        )  # >= 1 = 100
    ...
    def record_daily_grade(self, symbol: str, score: float, category: str):
        ...
        self.history[symbol]["grades"].append({"date": today, "score": score})
    ...
    def check_promotion(self, symbol: str, current_category: str) -> str | None:
        ...
        consecutive_85 = self.get_consecutive_days_above(
            symbol, self.VOLATILE_TO_ACTIVE
        )
    ...
    def check_demotion(
        self, symbol: str, current_category: str, held_assets: set = None
    ) -> str | None:
        """
