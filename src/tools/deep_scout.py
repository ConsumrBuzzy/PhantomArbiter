...
class DeepScout:
    ...
    def harvest_pools(self, token_mints: List[str]):
        ...
        Logger.info(
            f"🔎 [SCOUT] Harvesting pools for {len(valid_mints)} tokens (Bulk)..."
        )
        ...
        for i in range(0, len(valid_mints), CHUNK_SIZE):
            ...
            mint_str = ",".join(chunk)
            Logger.info(
                f"⚡ [SCOUT] Syncing batch {i // CHUNK_SIZE + 1} ({len(chunk)} tokens)..."
            )

            try:
                # V117.5: Bulk API execution
                resp = requests.get(
                    f"{self.dexscreener_url}{mint_str}", timeout=20, verify=False
                )
                if resp.status_code == 200:
                    ...
                    for p in pairs:
                        if p.get("chainId") == "solana":
                            ...
                            mapped_dex = None
                            if "raydium" in raw_dex:
                                mapped_dex = "RAYDIUM"
                            elif "orca" in raw_dex:
                                mapped_dex = "ORCA"
                            elif "meteora" in raw_dex:
                                mapped_dex = "METEORA"
                            elif "jupiter" in raw_dex:
                                mapped_dex = "JUPITER"
    ...
    def harvest_global_trending(self, min_vol=10000):
        ...
        Logger.info(
            f"🌐 [SCOUT] Scanning global network for volume spikes (>${min_vol})..."
        )
        try:
            ...
            if resp.status_code == 200:
                ...
                pairs = data.get("pairs", [])
                Logger.info(
                    f"📊 [SCOUT] Found {len(pairs)} candidates. Filtering for volume..."
                )
                mints = []
                for p in pairs[:500]:  # Deep scan top 500
                    ...
                    if vol >= min_vol:
                        ...
                        if mint:
                            mints.append(mint)
                            Logger.debug(
                                f"[SCOUT] Target Found: {symbol} (Vol: ${vol:,.0f})"
                            )
    ...
    def harvest_alpha(self, token_mint: str):
        ...
        if wallets:
            ...
            Logger.info(
                f"✅ [SCOUT] Added {len(wallets[:20])} Alpha Wallets to watchlist."
            )
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mode", choices=["pools", "alpha", "full", "global"], default="full"
    )
