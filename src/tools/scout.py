...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--scan-wallet", action="store_true", help="Scan wallet for untracked tokens"
    )
    ...
    if args.scan_wallet:
        ...
        for mint, bal in holdings.items():
            if bal > 0.01 and mint not in known_mints:
                ...
                symbol = scout.portfolio._fetch_token_symbol(
                    mint
                )  # Use helper from portfolio
                if symbol == "UNKNOWN":
                    symbol = input(
                        f"      ❓ Enter symbol for {mint[:8]}... > "
                    ).upper()
                else:
