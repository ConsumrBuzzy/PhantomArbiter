        ...
        try:
            ...
            for token in data[:50]:  # Check top 50, filter to limit
                if token.get("chainId") == "solana":
                    info = {
                        "symbol": token.get("tokenAddress", "")[:8],  # Will be resolved later
    ...
    def discover_and_validate(self, known_mints: set) -> list:
        ...
        for token in new_candidates[:3]:  # Limit to 3 per run (slow/safe)
            ...
            if result.is_safe:
                ...
                print(f"   ✅ {resolved_symbol}: SAFE (Liq: ${result.liquidity_usd:,.0f})")
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dry-run", action="store_true", help="Test without adding to assets")
