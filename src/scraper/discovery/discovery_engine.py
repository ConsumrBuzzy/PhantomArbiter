...
class DiscoveryEngine(BaseAgent):
    def __init__(self):
        ...
        self.MIN_ROI_AVG = (
            1.5  # 50% avg gain? Plan said 2x, let's stick to 2.0 per user request?
        )
        # User said >2x avg gain. That's high. Let's use 2.0.
        self.MIN_TRADES = 10  # Need history

        Logger.info(
            "[DISCOVERY] Engine initialized (Agent Protocol V1). Watchlist size: %d",
            len(self.watchlist),
        )
        ...
        while self.running:
            try:
                ...
                if score and score["is_smart_money"]:
                    Logger.info(
                        f"[DISCOVERY] 🧠 SMART MONEY FOUND! {wallet_address[:8]} (WR: {score['win_rate']:.2f}, ROI: {score['avg_roi']:.2f}x)"
                    )
            ...
            try:
                ...
                for mint in candidates:
                    ...
                    if score > 0:
                        ...
                        Logger.info(
                            f"[DISCOVERY] 🧠 Smart Money Signal on {symbol}: {score:.1f}"
                        )
    ...
    def _analyze_tx_pnl(self, tx: dict, wallet: str) -> Optional[float]:
        ...
        try:
            ...
            if wallet_idx == -1 or wallet_idx >= len(pre_balances):
                return None
