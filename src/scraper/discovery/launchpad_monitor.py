...
class LaunchpadMonitor:
    ...
    async def start(self) -> None:
        ...
        if not ws_url:
            Logger.error("   🔍 [DISCOVERY] HELIUS_WS_URL not set - cannot start monitor")
            self._running = False
            return
        ...
        Logger.info(f"   🔍 [DISCOVERY] Starting monitor for {len(program_ids)} programs...")
            ...
            while self._running:
                try:
                    async with websockets.connect(
                        ...
                        Logger.success("   🔍 [DISCOVERY] WebSocket connected to Helius")
    ...
    async def _subscribe_to_program(self, ws, program_id: str, sub_id: int) -> None:
        ...
        Logger.info(f"   🔍 [DISCOVERY] Subscribed: {platform.value} ({program_id[:8]}...)")
    ...
    async def _parse_logs(self, logs: List[str], signature: str, raw_value: Dict) -> None:
    ...
    async def _handle_create_event(
        ...
        def clean(val): return val.strip(",").strip()
        ...
        for log in logs:
            ...
            for i, part in enumerate(parts):
                ...
                # Mint
                if (lower_part == "mint:" or lower_part == "token:") and i + 1 < len(parts):
    ...
    async def _emit_launch(self, event: LaunchEvent) -> None:
        ...
        Logger.info(f"   🔍 [DISCOVERY] Launch: {event.symbol} on {event.platform.value}")
    ...
    async def _emit_migration(self, event: MigrationEvent) -> None:
        ...
        Logger.info(f"   🔍 [DISCOVERY] Migration: {event.mint[:16]}... -> {event.destination_dex}")
