...
class MetadataBackgroundResolver:
    ...
    def queue_token(self, mint: str, source: str = "UNKNOWN"):
        ...
        self.stats["queued"] += 1
        Logger.debug(f"[RESOLVER] Queued {mint[:8]}... from {source} (Priority: {is_priority}, retry in {delay}s)")
    ...
    def _process_pending(self):
        ...
        for pending in self.pending:
            ...
            if metadata and metadata.get("symbol") and not metadata["symbol"].startswith("UNK_"):
                # Success!
                self.resolved[pending.mint] = metadata
                self.stats["resolved"] += 1
                to_remove.append(pending)
                
                Logger.info(f"✅ [RESOLVER] Resolved {pending.mint[:8]}: {metadata['symbol']} ({metadata.get('name', '?')}) via {metadata.get('source', '?')}")
                ...
                if pending.attempts >= self.MAX_ATTEMPTS:
                    ...
                    self.stats["failed"] += 1
                    to_remove.append(pending)
                    Logger.debug(f"[RESOLVER] Failed permanently: {pending.mint[:8]}... after {pending.attempts} attempts")
                else:
                    pending.retry_at = now + self.RETRY_INTERVAL
                    Logger.debug(f"[RESOLVER] Retry scheduled for {pending.mint[:8]}... (attempt {pending.attempts})")
