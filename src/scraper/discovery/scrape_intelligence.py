...
class ScrapeIntelligence:
    ...
    def _scrape_signature(self, signature: str, dex: str) -> Optional[SwapIntel]:
        ...
        try:
            ...
            # Extract signer
            signer = data.get("signer", [""])[0] if isinstance(data.get("signer"), list) else data.get("signer", "")
    ...
    def _broadcast_whale_signal(self, intel: SwapIntel):
        ...
        Logger.info(f"🐋 [V84.0] WHALE: {intel.signer_wallet[:8]}... bought {intel.token_out[:8]}... on {intel.dex}")
        ...
        try:
            ...
            if intel.token_out:
                ...
                Logger.debug(f"[V85.1] MetadataResolver: Prioritizing {intel.token_out[:8]}... (whale vouch)")
        except:
            pass
