...
class ScoutManager:
    ...
    def run_hunt(self):
        ...
        for c in candidates:
            ...
            is_valid, reason, stats = self.gatekeeper.validate_candidate(c['address'], c['symbol'])
        ...
        msg = (f"[SCOUT] 🔍 Hunt Cycle Complete: {len(accepted)} Accepted, {total_rej} Rejected "
                ...
                for sym, info in data.get("assets", {}).items():
                    # Handle both simple mint string (legacy?) or object
                     ...
                     if isinstance(info, dict):
                         if info.get("mint") == mint: return True
