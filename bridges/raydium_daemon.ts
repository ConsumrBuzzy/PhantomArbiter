...
async function discoverPoolViaRaydiumV3(mintA: string, mintB: string): Promise<DiscoverResult> {
    ...
    try {
        ...
        return {
            ...
            volume24h: 0,
            feeRate: 0,
            type: 'standard', // Default on error
...
async function discoverPoolViaDexScreener(mintA: string, mintB: string): Promise<DiscoverResult> {
    ...
    try {
        ...
        return {
            ...
            type: 'standard' // DexScreener typically returns V4 pools as primary, though we should verify
        };
...
async function getPrice(poolAddress: string, existingRaydium?: Raydium): Promise<PriceResult> {
    try {
        ...
        if (!rpcResult || !rpcResult.poolInfo) {
            // If CLMM fails, try Standard AMM (V4)
        ...
        // If CLMM logic threw specific error, try Standard one last time or fail
        try {
            const std = await getStandardPoolPrice(poolAddress, existingRaydium?.connection || new Connection(RPC_URL, 'confirmed'));
...
async function getStandardPoolPrice(poolAddress: string, connection: Connection): Promise<PriceResult> {
    try {
        ...
        if (!info) {
            return {
                ...
                pool: poolAddress,
                tokenA: '', tokenB: '',
                priceAtoB: 0, priceBtoA: 0, liquidity: '0',
