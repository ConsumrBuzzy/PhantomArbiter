...
impl SignalScanner {
    ...
    /// This runs in Rust for max speed (no GIL overhead for iteration).
    fn scan_scalp_opportunities(&self, registry: Vec<SharedTokenMetadata>) -> Vec<ScalpSignal> {
        ...
        for token in registry {
            ...
            if token.velocity_1m > 0.05 { // > 5% per minute
                ...
                // Boosters
                if token.order_imbalance > 1.5 { confidence += 0.2; } // More buys than sells
                if token.spread_bps < 50 { confidence += 0.1; }       // Tight spread
            ...
            // If RSI < 20 (Oversold) -> Pump Reversion?
