...
class Settings:
    ...
    # V86.3: ML On-Demand
    ML_AUTO_RETRAIN = False           # False = Passive Mode (Alert Only), True = Active Retraining
    ...
    MAX_HOLD_TIME_MINUTES = 15  # Force-sell positions held longer than this (Scalping Mode)

    # V48.0: Capital Discipline
    MAX_CAPITAL_DRAWDOWN_PCT = 0.75  # Reset wallet if Equity drops 75% below Start Capital (25% Remaining)
    CLONE_WALLET_ON_FIRST_RUN = True  # If True, Paper Wallet starts with exact Real Wallet balance
    ...
    MEV_RISK_RATE_PCT = 0.15      # 15% chance of being sandwiched on high-value/visible trades
    ...
    SIMULATION_LOG_FILE = os.path.join(os.path.abspath(os.path.join(os.path.dirname(__file__), "../data")), "simulated_trades.csv")


    # ═══════════════════════════════════════════════════════════════════
    # V52.0: PUMP.FUN ADAPTER
    # ═══════════════════════════════════════════════════════════════════
    DYDX_ENABLED = os.getenv("DYDX_ENABLED", "false").lower() == "true"  # Read from .env
    ...
    EXECUTION_MODE = os.getenv("EXECUTION_MODE", "DEX")  # "DEX" (Solana) or "DYDX" (Perpetuals)
    ...
    ORCA_DEFAULT_POOL = "Czfq3xZZDmsdGdUyrNLtRhGc47cXcZtLG4crryfu44zE"  # SOL/USDC $32.5M TVL
