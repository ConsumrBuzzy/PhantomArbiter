...
def test_atomic_builder():
    ...
    cu_limit = phantom_core.estimate_compute_units(
        ops, num_accounts=12, num_signers=1, safety_margin_percent=10.0
    )
    print(f"   Ops: {ops}")
    ...
    try:
        ...
        payer_key_b58 = str(
            payer
        )  # Solders might default to something else, lets be specific
        ...
        tx_bytes = phantom_core.build_atomic_transaction(
            instruction_data, payer_key_b58, blockhash_str, rpc_slot, jito_slot
        )
